...
`schema.prisma`가 User, Player, Item, Equipment, PlayerProgress, PlayerSkill의 영속 스키마와 관계·인덱스를 정의한다. `migrations/0_init`은 기존 DB baseline이며 이후 디렉터리는 순차 변경을 담는다. `20260715000000_add_progress_and_skills`가 범용 진행 상태와 스킬 인스턴스 테이블을, `20260717000000_add_skill_experience`가 플레이어 스킬 경험치 필드를, `20260726000000_add_inventory_sort_order`가 영속 인벤토리 표시 순서를 추가한다. 생성 클라이언트는 `server/src/generated/prisma`를 대상으로 한다.
