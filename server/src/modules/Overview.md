...
- `minigame.ts`: 사용자별 단일 일회성 session/token/만료 상태와 타입별 서버 validator·완료/취소 callback. `fishing.ts`는 낚시 가능 장소·주손 낚싯대를 검사하고, 보조 미끼가 없으면 인벤토리 묶음을 자동 장착해 한 개만 소비한 뒤 입질 timer, 행운 등급, 검증 미니게임, 물고기/경험치 보상을 연결한다.
