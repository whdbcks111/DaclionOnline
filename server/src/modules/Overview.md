...
- `playerRegistry.ts`, `player.ts`, `location.ts`, `game.ts`, `coroutine.ts`: 순환 import 없는 온라인 Player 레지스트리와 Player/Progress/SkillBook 수명·dirty 저장, 통합 Location 오브젝트 HUD, 위치 태그·오브젝트 배치 검증/JSON 저장, 투사체를 포함한 월드 프레임/지연 실행.
