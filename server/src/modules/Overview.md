...
- `playerRegistry.ts`, `player.ts`, `location.ts`, `game.ts`, `coroutine.ts`: 순환 import 없는 온라인 Player 레지스트리와 Player/Progress/SkillBook 수명·dirty 저장, 현재 레벨과 상태효과 표시 snapshot을 포함한 HUD payload, 통합 Location 오브젝트/NPC ID 검증·JSON 저장, 투사체를 포함한 월드 프레임/제작 대기 실행. Player unload는 진행 중 제작과 NPC 대화를 종료한다.
