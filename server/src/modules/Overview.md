...
- `forging.ts`: `career:blacksmith` 정식 메인/서브/계보 조회와 구형 독립 flag의 비파괴 슬롯 이전, 가용 슬롯 전직 API, 대장장이 직업 또는 `metal_forging` 보유를 허용하는 단조 권한 API와 제련 소재 선검증, 제련 정밀도로 상한 45%까지 넓어지는 서버 권위 단조 리듬 판정, 실패 소재 파손/성공 조합형 장비 교환·경험치·이벤트 연결.
