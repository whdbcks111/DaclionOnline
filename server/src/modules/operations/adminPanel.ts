...
import { getAllLocations, getLocation } from '../../models/world/Location.js';
import Monster, { getAllMonsterData, getMonsterData } from '../../models/actors/Monster.js';
import Resource, { getAllResourceData } from '../../models/actors/Resource.js';
import { StatusEffectType } from '../../models/combat/StatusEffect.js';
import { StatType } from '../../models/core/Stat.js';
import { EquipSlotType } from '../../models/economy/Equipment.js';
import { AttributeType } from '../../models/core/Attribute.js';
import { markAllLocationsVisited } from '../../models/world/WorldMap.js';
import { discoverAllCraftingRecipes } from '../../models/professions/Crafting.js';
import type Player from '../../models/actors/Player.js';
import { fetchPlayerByUserId, getOnlinePlayers, getPlayerByUserId } from '../player/player.js';
import { getSession } from '../auth/login.js';
import { getIO } from '../infrastructure/socket.js';
import { broadcastBotMessageAll, broadcastNotification, sendNotificationToUser } from '../communication/message.js';
import logger from '../../utils/logger.js';
import { getMiniGamePresetSummaries, startMiniGamePreset } from '../professions/minigamePresets.js';
import {
...
import { cancelNavigation } from '../world/navigation.js';
import {
...
function formatRotationText(report: CombatRotationReport): string {
    return [
        ...
        `생명력: 본인 ${numberText(report.playerMaxLife)} / 대상 ${numberText(report.targetMaxLife)}`,
        `속도: ${numberText(report.currentSpeed)} : ${numberText(report.targetSpeed)} / 대상 회피 ${numberText(report.evasionChance * 100)}%`,
        `90% 회피 기준: 속도 ${numberText(report.evasionCapSpeed)} / 필요 민첩 ${report.evasionCapAgility}${report.evasionCapReached ? ' (현재 도달)' : ''}`,
        `${report.basicAttackType === 'magic' ? '마법 관통' : '방어 관통'}: ${numberText(report.penetration)} / 대상 방어 ${numberText(report.targetDefense)} → 유효 ${numberText(report.effectiveDefense)}`,
        `DPS: ${numberText(report.dps)} / 평균 환산: ${numberText(report.estimatedKillSeconds)}초 / 실제 로테이션 처치: ${numberText(report.simulatedKillSeconds)}초`,
        `최대 적중 단발: ${report.maxOpeningActionName} ${numberText(report.maxOpeningActionDamage)}${report.oneActionKill ? ' (치명타 포함 한 방 가능)' : ''}`,
        `첫 반격: ${numberText(report.counterattackDelay)}초 / 선공 ${report.openingBurstActions}행동 ${numberText(report.openingBurstDamage)}${report.killsBeforeCounterattack ? ` / ${numberText(report.openingBurstKillSeconds)}초 반격 전 처치` : ''}`,
        ...
        `피격 회피: ${numberText(report.defenderEvasionChance * 100)}% / 확정 회피 가동률 ${numberText(report.guaranteedEvasionCoverage * 100)}%`,
        `적 평타: ${numberText(report.incomingBasicDamage)} / ${numberText(report.incomingBasicInterval)}초`,
        ...(report.strongestIncomingSkillName ? [
            `최대 적 기술: ${report.strongestIncomingSkillName} ${numberText(report.strongestIncomingSkillDamage)}${report.strongestIncomingSkillUnavoidable ? ' (회피 불가)' : ''}${report.strongestIncomingSkillOneShots ? ' (즉사)' : ''}`,
        ] : []),
        `생존시간: 무회피 ${numberText(report.rawSurvivalSeconds)}초 / 속도 회피 ${numberText(report.evasionSurvivalSeconds)}초 / 방어기 포함 ${numberText(report.effectiveSurvivalSeconds)}초`,
        `처치 전: 예상 ${numberText(report.expectedIncomingHitsBeforeKill)}회 피격 / 피해 ${numberText(report.expectedIncomingDamageBeforeKill)} / 회복·보호막 ${numberText(report.projectedSupportBeforeKill)}`,
        `처치 후 예상 생명력: ${numberText(report.expectedLifeAfterKill)} / ${report.survivesUntilKill ? '생존' : '사망'}${report.evasionPreventsDeath ? ' (회피가 사망 방지)' : ''}`,
