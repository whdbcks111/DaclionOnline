...
`instanceDungeon.ts`는 10초 게이트, 참가자·절대 제한시간, 마녀의 주시/저주 동기화, 방 잠금, 귀환 균열과 동적 Location 폐기를 소유한다. 짧은 연결 끊김은 같은 Player로 복귀시키고 명시적 unload와 유예 만료는 원래 게이트로 이탈시킨 뒤 저장한다. DB에 원정 상태를 저장하지 않는다.
