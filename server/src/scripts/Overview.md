...
- `validateMasterData.ts`: DB 없이 아이템·상태효과 뒤에 연금술 조제약·정제수와 조합식 registry까지 bootstrap한 모든 코드 마스터를 로드하고 `locations.json`과 Lv.500 이후 생성 권역을 합친 참조·필수 아이콘을 검사한다. 연금술 재료·결과 아이템·상태효과와 양수 수량·조제약 사용 handler 참조도 `alchemy` scope로 검증하며 `npm run data:validate`에서 issue가 하나라도 있으면 종료 코드 1을 반환한다.
