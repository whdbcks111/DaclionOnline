...
- `reportBalance.ts`: 서버나 DB 없이 직업·스킬·아이템 마스터를 로드해 `Balance` 공개 API의 레벨별 1차 직업과 주력 장비·버프 아이템 전후 기준선을 출력한다. Lv.200 이상에서는 순서 있는 엘리트 조합 12개의 전용 액티브를 포함한 실측도 함께 출력한다. `npm run balance:report -- 50`처럼 사용하며 계수 변경 전후 결과를 비교한다.
