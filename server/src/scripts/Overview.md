...
- `reportBalance.ts`: 서버나 DB 없이 직업·스킬·아이템·몬스터 마스터를 로드해 개별 기준선과 추천 장비/동레벨 일반·보스/평타+전체 스킬 60초 로테이션을 출력한다. 기본 장비뿐 아니라 황혼왕릉과 유리모래 사막의 직업별 성장 장비도 같은 기준으로 비교한다. `npm run balance:report -- 50`은 단일 레벨, `-- all`은 Lv.20/50/100/150/200 및 Lv.200 엘리트 20조합을 비교한다.
