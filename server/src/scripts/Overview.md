...
- `reportBalance.ts`: 서버나 DB 없이 직업·스킬·아이템·몬스터 마스터를 로드해 개별 기준선과 추천 장비/동레벨 일반·보스/평타+전체 스킬 60초 로테이션을 출력한다. 기본 장비와 황혼왕릉부터 공허왕관까지의 직업별 성장 장비를 같은 기준으로 비교하고, 지역 전승 기술은 실제 상대 속도 회피·스킬 고유 관통·회피 불가·소모·쿨다운을 별도 출력한다. `npm run balance:report -- 50`은 단일 레벨, `-- all`은 Lv.20/50/100/150/200 및 Lv.200 엘리트 20조합을 비교한다.
