 ...
 */
export function resolveTagEffect(source: TagReadable, target: TagReadable): TagEffectResult {
    let match: TagEffectModifier | undefined

    for (const [sourceTag, targets] of modifiers) {
        if (!source.hasTag(sourceTag)) continue
        for (const [targetTag, modifier] of targets) {
            if (!target.hasTag(targetTag)) continue
...
export function applyTagEffectValue(value: number, source: TagReadable, target: TagReadable): TagEffectResult & { value: number } {
