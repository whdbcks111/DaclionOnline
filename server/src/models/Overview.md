...
- `Forging`: 클래스형 장비 형태·제련 재료, 리듬 정확도 효율과 서버 난수 trait를 조합해 이름·설명·내구도·인스턴스 modifier·재료 속성 태그가 담긴 장비 snapshot을 생성한다. 일반 제작품은 높은 확률로 재료+형태의 평범한 이름을 사용하고, 성향은 일반명 수식어와 희귀 조합형 고유명 어휘군 모두에 반영된다.
