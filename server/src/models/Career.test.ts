...
test('1차 직업은 최소 3개 스킬을 지급하고 서로 다른 12개 순서 조합만 엘리트 직업을 가진다', () => {
    ...
    assert.ok(eliteJobs.every(job => job.grantedSkills.length === 1
        && getSkillData(job.grantedSkills[0].skillDataId)?.tags.includes('skill:passive')));
...
test('직업 스킬 설명은 현재 수치와 계수 hover를 제공하고 두 발동 방식을 안내한다', () => {
    ...
    for (const skillDataId of getAllJobs().filter(job => job.tier === JobTier.ELITE)
        .flatMap(job => job.grantedSkills.map(grant => grant.skillDataId))) {
        const skill = new Skill({ playerId: player.userId, skillDataId });
        assert.match(skill.formatDescription(player), /\[tooltip=/);
        assert.equal(skill.isPassive, true);
    }
});
