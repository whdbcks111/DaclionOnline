...
import { LegacyStatusEffects } from '../../data/combat/statusEffects.js';
...
test('열병과 빙결도 남은 세기에 따라 양방향으로 상쇄된다', () => {
    const sunFever = LegacyStatusEffects.SUN_FEVER;
    const frozenWins = new InteractionEntity('냉기가 이기는 대상');
    frozenWins.applyStatusEffect(LegacyStatusEffects.FROZEN, 10, 3);
    assert.equal(frozenWins.applyStatusEffect(sunFever, 5, 1).action, StatusEffectApplyAction.REJECTED);
    assert.equal(frozenWins.getStatusEffect(LegacyStatusEffects.FROZEN)?.duration, 10 - (5 / 3));

    const feverWins = new InteractionEntity('열기가 이기는 대상');
    feverWins.applyStatusEffect(sunFever, 10, 4);
    const frozen = feverWins.applyStatusEffect(LegacyStatusEffects.FROZEN, 10, 2);
    assert.equal(frozen.action, StatusEffectApplyAction.REJECTED);
    assert.equal(feverWins.getStatusEffect(sunFever)?.duration, 5);
    assert.equal(feverWins.hasStatusEffect(LegacyStatusEffects.FROZEN), false);
});
