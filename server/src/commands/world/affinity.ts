...
import { getTagEffectAffinitySnapshots } from '../../models/combat/TagEffect.js'
import type { TagEffectDisplayRelation } from '../../models/combat/TagEffect.js'
...
export function initAffinityCommands(): void {
    registerCommand({
        ...
        handler(userId) {
            sendBotMessageToUser(userId, buildAffinityMessage())
        },
    })
}

/** 내부 tag key 없이 표시명·아이콘·실제 배율만 담은 사용자용 속성표. */
export function buildAffinityMessage() {
    return chat()
        .text('[ 속성표 ]  ')
        .hide('상성표 보기', builder => {
            builder.text('[ 속성표 ]\n')
