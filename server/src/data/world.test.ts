...
test('월드 맵 연결과 오브젝트 정의가 유효하고 고블린이 남아 있지 않다', () => {
    ...
    assert.deepEqual(
        ...
        { safe: 6, neutral: 13, hostile: 6 },
    );
    assert.deepEqual(
        locations.filter(location => location.zoneType === 'hostile').map(location => location.id).sort(),
        ['crystal_gallery', 'crystal_throne', 'deep_shaft', 'swamp_heart', 'volcanic_core', 'volcanic_crater'],
    );
