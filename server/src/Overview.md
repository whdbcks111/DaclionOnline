...
`index.ts`가 Socket.io를 먼저 초기화한 뒤 인증, 채팅, 명령, 플레이어, 위치, 업로드 미디어 정리와 게임 루프를 조립하고 HTTP 라우트와 정적 파일을 연결한다. 아이템·몬스터·자원·투사체·상점·상성·통계·스킬·제작법·NPC 데이터 모듈은 시작 시 정적 import로 레지스트리에 등록하며, 참조 대상 정의가 먼저 로드되도록 순서를 유지한다.
