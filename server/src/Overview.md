...
`index.ts`가 Socket.io를 먼저 초기화한 뒤 인증, 채팅, 명령, 플레이어, 위치, 게임 루프를 조립하고 HTTP 라우트와 정적 파일을 연결한다. 아이템·몬스터·자원·투사체·상점·상성·통계·스킬·제작법 데이터 모듈은 시작 시 정적 import로 레지스트리에 등록하며, 스킬 자동 획득이 참조할 수 있도록 통계 정의를 먼저 로드한다.
