...
- `Home.tsx`: 채팅, 채널, 슬래시 명령·슬래시 없는 별칭의 명령/인자 자동완성, HUD 데이터의 중심 조정자. 전송 옆 공개/비공개 버튼은 서버의 정보 열람 모드와 동기화한다. 메시지 전송 시 contenteditable 포커스를 유지해 모바일 키보드가 닫히거나 깜빡이지 않게 한다.
