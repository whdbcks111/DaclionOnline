...
- `statusEffectVisuals.ts`: 상태효과 ID를 화염·독·빙결·감전·출혈·어둠성 저주·석화 화면 preset, 독성/출혈 HP 색과 마비성 메시지 교란으로 변환하는 클래스형 enum과 resolver.
...
새 주요 상태효과는 서버 ID를 `StatusScreenEffectPreset` 또는 메시지 교란 ID 집합에 등록하고, 새 시각 계열이 필요할 때만 `client/public/effects/status/` 에셋을 추가한다. 단순 수치 버프는 장시간 화면을 점유하지 않도록 HUD 아이콘만 사용한다.
