...
`WorldMapNode`는 방문 장소를 대표색별 영역으로 묶고 발견된 연결을 넓은 캡슐형 면으로 이어 경로·노드 아래 연속된 바이옴 레이어를 만든다. 색이 다른 인접 영역은 연결 중간에서 만나며 단색 내부는 유지하고 외곽에만 SVG 변형·페더링을 적용한다. 미방문 장소는 색을 렌더링하지 않는다.
