...
| Alchemy | `startAlchemy`, `createAlchemyTrackingConfig`, `createAlchemyCompletionOutputs`, `resolveAlchemyPotionTargets`, `useAlchemyPotion`; model `resolveAlchemyPotionUse`, `recordAlchemyReagentExperiments`, `hasExperimentedAlchemyReagent`, `getAlchemyReagentInsight` | ready 시 재료·정제수 원자 소비와 reagent별 PlayerProgress 실험 flag를 함께 확정하고, 감각 200/300/400/500 단계의 가공 정보 snapshot을 제공한다. 서버 proof 품질 결과와 중량 초과 바닥 보존, formula/result item/품질 canonical 재구성, metadata 위변조·장소·파티·몬스터 대상 거부를 담당한다. |
