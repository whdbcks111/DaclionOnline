...
| `Monster` | `activateSkill`, `recordThreat`, `taunt`, `getThreatContributions`, `getInspectionSnapshot`, `update`, `onDeath`, `rollDrops`, `rollGold` | MonsterData.ai 기반 마지막 공격자/누적 위협 대상 선택, 피해·지원 기여도 보상 귀속, 실제 SkillData 순환 패턴과 보상·리스폰 |
...
| `Location`, `RegionRiskPolicy` | `riskPolicy`, `values/fromKey/require`, 사망 손실/부활 계산, `hasTag`, `getObjects/getObject/getObjectCount/getAttackableObjects/hasObject/isResourceDefeated`, `addObject/removeObject`, `getNpcs/getNpc/hasNpc`, `addDroppedItem`, `getDroppedItems`, `pickupItem/pickupAllItems`, `getAvailableConnections/findAvailableConnection`, `update` | 안전/중립/적대 PVP·사망 정책, Monster/Resource 통합 오브젝트와 퍼즐 자원 파괴 판정, 다중 공격 대상 snapshot, ID 기반 NPC, 유연한 장소 입력, 공개 가능한 `lockReason`, 동일 상태 바닥 아이템의 maxStack 병합·조회·회수 |
