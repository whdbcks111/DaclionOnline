...
| `Monster` | `activateSkill`, `getInspectionSnapshot`, `update`, `onDeath`, `rollDrops`, `rollGold` | Entity 공통 공격자 타게팅을 사용하는 AI, 감정용 설명·전투·보상 DTO, 느린 기본 공격과 실제 SkillData 순환 패턴, 물리/마법 공격 프로필과 적중 상태이상, 보상, 리스폰 |
