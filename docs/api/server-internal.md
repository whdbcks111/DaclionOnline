...
| `Skill`, `SkillBook` | `get/set/resetMetadata`, `get/setActiveState`, `getCalculatedField`, `getExperienceGain/getRequiredExperience/addExperience`, `formatDescription/Cost/ActivationCondition`, `createRuntime`, `grant`, `activateByInput/ById/FromMessage`, `getHudSnapshots`, `hasActiveSkill`, `getActivationStatus`, `update`, `finishAll`, `load/save` | `{{icon.attributeKey}}` 능력치 계수 아이콘, 직업·무기 조건, 메시지 시전어와 성공 후 경험치·레벨업 및 `activationFeedback`, 표시 가능한 스킬 HUD DTO, delta·계산 템플릿과 영속/비영속 lifecycle |
