...
| Location service | `loadLocationsFromJson`, `updateLocations(dt, onlinePlayers)`, `initLocation` | JSON/소켓/프레임 조정과 온라인 플레이어 기반 보스방 감지 |
...
| `Player` | `career`, `titles`, `loadByUserId`, `create`, `save`, `getAttackDeniedReason`, `applyRegionDeathPenalty`, `performBasicAttack`, `receiveLoot`, `equipInventoryItem`, `canSpendMentality/spendMentality/restoreMentality`, `gainExp(amount, options?)`, `allocateStat`, `adjustLevel`, `calculateLevelSurvivalCapacityBonus` | CareerProfile과 TitleBook을 포함한 영속 aggregate, 지역/파티 PVP 검증과 사망 손실, 적대 구역 사망 시 자동 두루마리 한 장으로 지역·카르마 포함 부활 대기를 절반으로 줄인 절대 deadline 영속, 다음 사망 판정까지만 처치 경험치를 보호하는 선택적 경험치 지급, 중량 초과 전리품의 현재 장소 보존, 안전한 인벤토리→장비 교환, 무기 오버라이드·적중 callback 기본 공격과 스킬 자원·실제 성장 지급분을 동반하는 관리자 레벨 조정, Lv.1 이후 레벨당 최대 배고픔·수분 +1 파생 계산 |
