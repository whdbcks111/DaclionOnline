...
| Party | `partyManager.invite/accept/decline/leave/disband/kick/removeDisconnectedPlayer/getParty/areInSameParty/getHudData/distributeMonsterExp`, `calculatePartyExpGrant` | 내부 Map을 숨긴 초대·구성·PVP 아군 판정·HUD·같은 장소 몬스터 경험치 공유와 즉시 사망 패널티 보호 옵션 전달 |
...
| `Monster` | `activateSkill`, `recordThreat`, `engageIntruder`, `taunt`, `getThreatContributions`, `getCombatClaimUserIds`, `getInspectionSnapshot`, `update`, `onDeath`, `rollDrops`, `rollGold` | MonsterData.ai 기반 마지막 공격자/누적 위협 대상 선택, 중복 없는 보스방 최초 감지, 최초 공격자·당시 파티 교전 선점, 피해·지원 기여도 보상 귀속, 실제 SkillData 순환과 취소 가능한 미니게임 패턴·보상·리스폰 |
