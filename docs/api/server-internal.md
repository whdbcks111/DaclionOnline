...
| 모델/레지스트리 | 주요 API | 용도 |
| --- | --- | --- |
| `Entity` | `attackOwner`, `getDefeatCreditUserIds`, `getEvasionAttackSpeed`, `acquireCombatTarget`, `recordCombatEngagement`, `isDefeated/defeatLabel`, `getAttackDeniedReason`, `grant/remove/consumeGuaranteedEvasion`, `isInteractable/interact`, `hasTag/getTags`, `damage/heal(amount, source?)`, `restoreHunger/restoreThirst`, `canAttack/attack(AttackOptions)`, `setShield(..., source?)/get/has/remove/clearShield(s)`, `getTotalShield/getShieldDisplaySnapshots/getShieldBarSegments`, 상태효과·행동 제한·lifecycle API | 공격 pipeline과 공격 종류별 속도 회피·고정 피해, 실제로 받아들여진 공격·회피·source 피해의 플레이어/몬스터 최종 owner 교전 전달과 자원 제외, 처치 퀘스트 추가 수신자의 기본 빈 snapshot, `combatMessage` 기술명·상황 코멘트, 선택적 `miningPower` 채굴력과 `armorDurabilityDamageMode` 손상 범위 전달, 실제 생명력 피해만 사용하는 방어구 손상, 타입별 보호막, source가 명시된 실제 치유량과 보호막 실제 흡수량의 지원 위협·기여 전파, 직접·지속 피해 확정 뒤 공통 확장 경계, 최대값 내 자원 회복, source별 상태효과·행동 제한과 공통 생명주기 |
...
| Balance | `BALANCE_PROFILE_LEVELS`, `createBalanceScenario(level, mainJobId, subJobId?, encounter?, loadoutItemDataId?)`, `analyzeSkill/Job/ItemBalance`, `analyzeCombatRotation`, `analyzeBalanceProfile`, `analyzeAllBalanceProfiles` | Lv.20~1000 공용 회귀, 추천 또는 명시 주무기, 몬스터 프로필 공식으로 정규화한 대상, 일반 30초·보스 240초의 평타+가용 스킬 공유 자원 및 0.9초 전투 기술 전역 연계 로테이션 진단 |
| Threat | `ThreatAction.values/fromKey`, `MonsterAiDisposition.values/fromKey`, `normalizeMonsterAiProfile`, `ThreatTable.getContributionSnapshots/getDefeatContributionSnapshot`, `reportSupportThreat` | 마스터 AI 성향·지능·행동 가중치·도발 저항, `ThreatContributionSnapshot.threat`의 현재 AI 위협 점수, 감쇠·이탈 제거되는 대상 선택용 위협도와 userId 기반 불변 처치 기여 원장의 분리 추적 |
...
| `Player` | `career`, `titles`, `codex`, `musicCombatState`, `loadByUserId`, `create`, `save`, `getAttackDeniedReason`, `applyRegionDeathPenalty`, `performBasicAttack`, `receiveLoot`, `equipInventoryItem`, `getItemEquipmentAttributePreview`, `canSpendMentality/spendMentality/restoreMentality`, `gainExp(amount, options?)`, `allocateStat`, `adjustLevel`, `previewAllocatedStatReset/resetAllocatedStats`, `calculateLevelSurvivalCapacityBonus` | CareerProfile·TitleBook·CodexBook을 포함한 영속 aggregate, 별도 9초 일반/보스 타이머를 사용하는 비영속 `exploration/combat/boss` 음악 상태와 이동·사망·부활 초기화, 지역/파티 PVP 검증과 사망 손실, 적대 구역 사망 시 자동 두루마리 한 장으로 지역·카르마 포함 부활 대기를 절반으로 줄인 절대 deadline 영속, 다음 사망 판정까지만 처치 경험치를 보호하는 선택적 경험치 지급, 중량 초과 전리품의 현재 장소 보존, 실제 장착과 슬롯 선택을 공유하는 읽기 전용 장비 최종 능력치 미리보기, 안전한 인벤토리→장비 교환, 무기 오버라이드·적중 callback 기본 공격과 스킬 자원·실제 성장 지급분을 동반하는 관리자 레벨 조정, 자동 성장 하한을 보존하는 직접 분배 스탯 환급, Lv.1 이후 레벨당 최대 배고픔·수분 +1 파생 계산 |
...
| `Monster` | `activateSkill`, `recordThreat`, `engageIntruder`, `taunt`, `getThreatContributions`, `getDefeatContributionSnapshot`, `getDefeatCreditUserIds`, `getCombatClaimUserIds`, `getInspectionSnapshot`, `update`, `onDeath`, `rollDrops`, `rollGold` | MonsterData.ai 기반 마지막 공격자/누적 위협 대상 선택, 중복 없는 보스방 최초 감지, 최초 공격자·당시 파티 교전 선점, 실제 피해·치유·보호막 흡수·제어의 불변 처치 기여 snapshot과 경험치 분배, 양수 기여 userId의 중복 없는 처치 퀘스트 snapshot, 기존 주 기여자/막타 전리품 귀속, 실제 SkillData 순환과 취소 가능한 미니게임 패턴·보상·리스폰 |
| Monster stats | `MonsterStatProfile.values/fromKey/fromInput`, `MonsterRank.values/fromKey/fromInput`, `calculateMonsterBaseAttributes`, `inferMonsterStatProfile` | `300+75L+4L²+0.002L³` 생명력과 고레벨 점근형 보스 배율을 균형/근접/수호/기동/마법/복합 역할과 일반/정예/필드 보스/보스 체급에 배분하고 고유 weight·최종 override를 적용 |
...
| `StatusEffectType`, `StatusEffect` | `define/values/fromKey/fromInput`, `icon`, `source`, `formatDescription`, `get/set/resetMetadata`, `upgrade/refreshDuration/replaceSource`, lifecycle callback | 공통 레벨 상한 없는 아이콘 key 효과 정의, 대상별 duration/level/metadata delta와 최종 공격 소유자 source, 성공한 명시적 재적용에서만 source를 교체하는 병합, 지속 피해·치유·실제 제어 시간 귀속과 tick 처리 |
