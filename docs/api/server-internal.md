...
| Alchemy | `startAlchemy`, `createAlchemyTrackingConfig`, `createAlchemyCompletionOutputs`, `resolveAlchemyPotionTargets`, `useAlchemyPotion`; model `resolveAlchemyPotionUse` | ready 시 재료·정제수 원자 소비, 서버 proof 품질 결과와 중량 초과 바닥 보존. 포션 사용은 formula/result item/품질 입력에서 canonical 효과를 재구성하고 metadata 위변조·장소·파티·몬스터 대상을 소비 전에 거부한다. |
