...
| `Player` | `career`, `titles`, `loadByUserId`, `create`, `save`, `getAttackDeniedReason`, `applyRegionDeathPenalty`, `performBasicAttack`, `receiveLoot`, `equipInventoryItem`, `canSpendMentality/spendMentality/restoreMentality`, `gainExp(amount, options?)`, `allocateStat`, `adjustLevel`, `calculateLevelSurvivalCapacityBonus` | CareerProfile과 TitleBook을 포함한 영속 aggregate, 지역/파티 PVP 검증과 사망 손실, 다음 사망 판정까지만 처치 경험치를 보호하는 선택적 경험치 지급, 중량 초과 전리품의 현재 장소 보존, 안전한 인벤토리→장비 교환, 무기 오버라이드·적중 callback 기본 공격과 스킬 자원·실제 성장 지급분을 동반하는 관리자 레벨 조정, Lv.1 이후 레벨당 최대 배고픔·수분 +1 파생 계산 |
...
| `EquipSlotType`, `Equipment` | `values/fromKey/fromInput`, `getEquipped`, `getAllEquipped`, `hasEquippedItemTag`, `setItemMetadata/resetItemMetadata`, 내구도 API, `damageArmorDurability`, `equip/equipSwap/unequip/consumeEquippedItem`, `applyModifiers/applyOwnerEffects`, `triggerOwnerDefeatedEntity/triggerDamageTakenEffects/updateOwnerEffects/tryPreventFatalDamage`, `load/save` | 슬롯·modifier·내구도·dirty 영속성과 경험치 배율, 처치·피격·지속·치명적 피해·장비 파괴 특수 효과의 공개 실행 경계 |
