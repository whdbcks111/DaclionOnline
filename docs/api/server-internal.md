...
| `Player` | `career`, `titles`, `codex`, `musicCombatState`, `loadByUserId`, `create`, `save`, `getAttackDeniedReason`, `applyRegionDeathPenalty`, `performBasicAttack`, `receiveLoot`, `equipInventoryItem`, `canSpendMentality/spendMentality/restoreMentality`, `gainExp(amount, options?)`, `allocateStat`, `adjustLevel`, `previewAllocatedStatReset/resetAllocatedStats`, `calculateLevelSurvivalCapacityBonus` | CareerProfile·TitleBook·CodexBook을 포함한 영속 aggregate, 별도 9초 일반/보스 타이머를 사용하는 비영속 `exploration/combat/boss` 음악 상태와 이동·사망·부활 초기화, 지역/파티 PVP 검증과 사망 손실, 적대 구역 사망 시 자동 두루마리 한 장으로 지역·카르마 포함 부활 대기를 절반으로 줄인 절대 deadline 영속, 다음 사망 판정까지만 처치 경험치를 보호하는 선택적 경험치 지급, 중량 초과 전리품의 현재 장소 보존, 안전한 인벤토리→장비 교환, 무기 오버라이드·적중 callback 기본 공격과 스킬 자원·실제 성장 지급분을 동반하는 관리자 레벨 조정, 자동 성장 하한을 보존하는 직접 분배 스탯 환급, Lv.1 이후 레벨당 최대 배고픔·수분 +1 파생 계산 |
| `AttributeType`, `Attribute` | `values/fromKey`, `icon/iconMarkup`, `summarizeAttributeModifiers`, `get`, `setBase`, `addModifier(s)`, `removeBySource` | 투사체 가속·채굴력을 포함한 전체 능력치의 대표색 아이콘·스킬 포맷 문법·표시 메타데이터, 기본값 + add/multiply 수정자 계산과 능력치별 고정 합·배율 곱 표시 요약 |
...
| `EquipSlotType`, `ArmorDurabilityDamageMode`, `Equipment` | `values/fromKey/fromInput`, `calculateArmorDurabilityDamageChance`, `getEquipped`, `getAllEquipped`, `hasEquippedItemTag`, `setItemMetadata/resetItemMetadata`, 내구도 API, `damageArmorDurability(lifeDamage, maxLife, mode, random?)`, `equip/equipSwap/unequip/consumeEquippedItem`, `applyModifiers/applyOwnerEffects`, `triggerOwnerDefeatedEntity/triggerDamageTakenEffects/updateOwnerEffects/tryPreventFatalDamage`, `load/save` | 슬롯·modifier·내구도·dirty 영속성과 경험치 배율, 실제 생명력 피해 기반 10~70% 단일 부위 가중 손상과 명시적 전 부위 손상의 불변 snapshot, 처치·피격·지속·치명적 피해·장비 파괴 특수 효과의 공개 실행 경계 |
