...
| `Item`, Item registry | `MAX_STACKABLE_ITEM_COUNT/MAX_ITEM_REINFORCEMENT_LEVEL`, `calculateItemReinforcementRate`, `getReinforcementBaseModifiers/getReinforcementModifiersAtLevel`, `basicAttackOverrideKey`, `attackEffects/triggerInstanceAttackEffects/rollInstanceAttackDamageMultiplier`, `ItemData.gameplayEffects`와 공격·피격·처치·tick·치명상·파괴 callback, 이미지·내구도·metadata·태그·snapshot·registry API | 사실상 무제한 스택, 원본 능력치 비례 +15 강화 계산, 감정용 고유 효과, 장비 소유자 특수 효과, 인스턴스 상태와 영속 delta |
