...
| `Player` | `career`, `titles`, `codex`, `musicCombatState`, `loadByUserId`, `create`, `save`, `getAttackDeniedReason`, `applyRegionDeathPenalty`, `performBasicAttack`, `receiveLoot`, `equipInventoryItem`, `canSpendMentality/spendMentality/restoreMentality`, `gainExp(amount, options?)`, `allocateStat`, `adjustLevel`, `calculateLevelSurvivalCapacityBonus` | CareerProfile·TitleBook·CodexBook을 포함한 영속 aggregate, 별도 9초 일반/보스 타이머를 사용하는 비영속 `exploration/combat/boss` 음악 상태와 이동·사망·부활 초기화, 지역/파티 PVP 검증과 사망 손실, 적대 구역 사망 시 자동 두루마리 한 장으로 지역·카르마 포함 부활 대기를 절반으로 줄인 절대 deadline 영속, 다음 사망 판정까지만 처치 경험치를 보호하는 선택적 경험치 지급, 중량 초과 전리품의 현재 장소 보존, 안전한 인벤토리→장비 교환, 무기 오버라이드·적중 callback 기본 공격과 스킬 자원·실제 성장 지급분을 동반하는 관리자 레벨 조정, Lv.1 이후 레벨당 최대 배고픔·수분 +1 파생 계산 |
...
| `Inventory`, `InventorySortMode` | `values/fromKey/fromInput`, `createEmpty`, `getItem*`, `getIndexedItems`, `getFirstItemByData`, `getCount/countMatching/removeMatching`, `sortItems`, `selectItems`, `replaceSelectedItems`, `subscribeChanges`, `set/resetItemMetadata(ByIndex)`, `setItemDurability`, `changeItemDurability`, `increaseItemDurability`, `decreaseItemDurability`, `canAdd`, `canAddSnapshot(s)`, `addItem`, `addItemSnapshot`, `restoreItemSnapshot`, `useItem`, `removeItem*`, `removeItemInstance`, `clear`, `load`, `save` | 종류·이름·사용 가능/내구도 우선순위 정렬과 `sort_order` 영속성, metadata·내구도 dirty 추적과 내구도 0 인스턴스 파괴, 신규 item ID에도 안전한 인덱스 UI 변경, 전체 영속 삭제, raw 배열 없는 predicate 수량·일괄 제거/변경 구독, 겹치는 재료 predicate의 중복 없는 선택과 선검증 교환, 무게/스택/사용/태그 포함 이동/DB 동기화. 일반 stackable 아이템은 중량이 실질 한계이며 load가 과거의 동일 상태 분할 스택을 현재 안전 상한까지 병합한다. `restoreItemSnapshot`은 이미 소유했던 런타임 에스크로를 취소할 때 현재 중량과 무관하게 원상 복구한다. |
