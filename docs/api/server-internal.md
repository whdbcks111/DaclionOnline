...
| Monster stats | `MonsterStatProfile.values/fromKey/fromInput`, `MonsterRank.values/fromKey/fromInput`, `calculateMonsterBaseAttributes`, `inferMonsterStatProfile` | 레벨 성장 예산을 균형/근접/수호/기동/마법/복합 역할과 일반/정예/필드 보스/보스 체급에 배분하고 고유 weight·최종 override를 적용 |
| Monster registry | `defineMonster`, `getMonsterData`, `getAllMonsterData`, `Monster.getDisplayIcon`, `registerMonsterChallengePattern/hasMonsterChallengePattern` | Lv.1~500 몬스터 마스터 데이터, `monsters/{id}` 표시 초상과 handler key 기반 보스 미니게임 패턴 |
