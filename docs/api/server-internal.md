...
| `Resource` | `miningHardness`, `evaluateMiningImpact`, `isInteractable`, `interactionCooldownRemaining`, `interact`, `rollInteractionCooldown`, `resetInteractionCooldown`, `getAttackDeniedReason`, `onDeath`, `rollDrop`, `rollExp` | 비공격 Entity 자원, 경도와 공격 타입·채굴력에 따른 피해 효율, 상호작용 쿨타임과 운영 초기화, 가중치 드롭·범위 경험치·리스폰 |
