...
| `Monster` | `activateSkill`, `recordThreat`, `engageIntruder`, `taunt`, `getThreatContributions`, `getDefeatContributionSnapshot`, `getCombatClaimUserIds`, `getInspectionSnapshot`, `update`, `onDeath`, `rollDrops`, `rollGold` | MonsterData.ai 기반 마지막 공격자/누적 위협 대상 선택, 중복 없는 보스방 최초 감지, 최초 공격자·당시 파티 교전 선점, 실제 피해·치유·보호막 흡수·제어의 불변 처치 기여 snapshot과 경험치 분배, 기존 주 기여자/막타 전리품 귀속, 실제 SkillData 순환과 취소 가능한 미니게임 패턴·보상·리스폰 |
...
| Monster registry | `defineMonster`, `getMonsterData`, `getAllMonsterData`, `Monster.getDisplayIcon`, `registerMonsterChallengePattern/hasMonsterChallengePattern` | Lv.1~1000 몬스터 마스터 데이터, `monsters/{id}` 표시 초상과 handler key 기반 보스 미니게임 패턴 |
