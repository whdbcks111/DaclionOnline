...
`components/minigame/MiniGameOverlay`는 서버 `miniGameStart`를 전체 화면 overlay로 렌더링한다. 낚시는 공용 `simulateFishingCapture`를 사용해 화면을 미리 계산하지만 성공 권한은 서버에 있으며, 키보드와 pointer 조이스틱의 축 변경을 20ms 단위로 합친 뒤 전송 시점의 불변 trace snapshot만 `miniGameResult`로 반환한다.
