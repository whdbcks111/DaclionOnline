...
권한 10 이상의 운영자가 햄버거 메뉴의 `관리자 페이지` 버튼으로 `/admin`을 새 탭에서 연다. 클라이언트에서 버튼을 감추는 것은 편의 기능이며, 모든 조회와 변경은 `server/src/modules/operations/adminPanel.ts`가 세션 권한을 다시 검사한다.

## 화면 구성

- 플레이어 관리: 전체 캐릭터를 조회하되 현재 접속자를 먼저 정렬한다. 닉네임·계정·UID 검색과 상세 상태, 인벤토리/장비, 스탯/스킬/칭호/상태이상 검사를 제공한다.
- 플레이어 작업: 순간이동, 카르마·레벨·스탯·직업·자원 설정, 아이템 지급·삭제, 보유 스킬 레벨 설정·삭제, 칭호 부여·삭제, 상태이상 관리, 부활, 전체 지도와 제작법 발견 처리, 선택한 온라인 플레이어 알림을 카테고리별로 실행한다. 칭호 부여 목록은 미보유 칭호, 삭제 목록은 보유 칭호만 표시하며 장착 칭호를 삭제하면 패시브도 즉시 정리된다. 선택 상세에는 자연 감소가 반영된 현재 카르마와 단계가 표시되며 `카르마 설정`은 `Player.setKarma()`를 사용한다. `레벨 설정`은 값만 덮어쓰고, `레벨 조정`의 상승은 기존 스탯 분배를 정규화하지 않은 채 상승 레벨마다 모든 스탯 +1과 가용 포인트 +3만 더한다. 하락만 가용 포인트를 먼저 회수하고 부족분은 새 레벨의 자동 성장 최저값을 지키면서 기존 분배 비율에 가깝게 회수한다. 테스트 카테고리에서는 낚시와 일반 위험 회피, 수정 연쇄 낙석, 심장수호자 공명 폭주, 성계 교차포화 미니게임 프리셋을 보상 없이 실행하며 사람 확인 화면의 강제 실행·오탐 해제도 제공한다.
...
- 밸런스 분석: 검색 가능한 스킬·직업·장비/버프 아이템 선택창과 `전투 로테이션 프로파일`로 `Balance` 공개 API를 실행한다. 프로파일은 추천 장비, 평타+전체 성장 스킬, 공유 정신력/쿨다운, 동레벨 일반·보스 대상으로 평균 DPS·실제 처치 시각·최대 단발·첫 반격 전 폭딜과 양쪽 HP·피격 회피·확정 회피·보스 기술·단계별 생존시간을 표시한다.
...
1. `adminPanelRequestBootstrap`으로 아이템·밸런스 분석 대상 아이템·스킬·칭호·직업·장소·몬스터·상태이상·스탯·미니게임 프리셋의 표시용 option 목록을 받는다.
...
`client/src/components/dialog`의 `Dialog`는 portal, Escape/배경 닫기, 포커스 복원과 body 스크롤 잠금을 제공한다. `FormDialog`는 필드 정의 배열로 text, number, select, textarea, checkbox 입력과 비동기 제출 상태를 만들며 필수 select는 첫 option을 기본값으로 선택한다. 모든 select는 label·내부 코드·설명을 검색할 수 있는 공용 `SearchableSelect`를 사용한다. 선택 목록도 `document.body` portal로 분리해 다이얼로그 본문과 footer에 잘리지 않으며, PC에서는 선택 버튼 위·아래의 viewport 여유 공간을 계산해 더 넓은 방향으로 열고 모바일에서는 화면 안쪽 하단 목록으로 표시한다. 이후 관리자 작업이나 화면 오버레이 입력은 별도 모달을 만들기보다 이 API를 우선 재사용한다.
