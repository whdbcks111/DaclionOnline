...
- 플레이어 작업: 순간이동, 레벨·스탯·직업·자원 설정, 아이템 지급·삭제, 보유 스킬 레벨 설정·삭제, 상태이상 관리, 부활, 전체 지도와 제작법 발견 처리, 선택한 온라인 플레이어 알림을 카테고리별로 실행한다. 테스트 카테고리에서는 낚시와 일반 위험 회피, 수정 연쇄 낙석, 심장수호자 공명 폭주, 성계 교차포화 미니게임 프리셋을 보상 없이 실행한다.
