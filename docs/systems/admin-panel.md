...
- 플레이어 작업: 순간이동, 카르마·레벨·스탯·직업·자원 설정, 아이템 지급·삭제, 보유 스킬 레벨 설정·삭제, 상태이상 관리, 부활, 전체 지도와 제작법 발견 처리, 선택한 온라인 플레이어 알림을 카테고리별로 실행한다. 선택 상세에는 자연 감소가 반영된 현재 카르마와 단계가 표시되며 `카르마 설정`은 `Player.setKarma()`를 사용한다. `레벨 설정`은 값만 덮어쓰고, `레벨 조정`의 상승은 기존 스탯 분배를 정규화하지 않은 채 상승 레벨마다 모든 스탯 +1과 가용 포인트 +3만 더한다. 하락만 가용 포인트를 먼저 회수하고 부족분은 새 레벨의 자동 성장 최저값을 지키면서 기존 분배 비율에 가깝게 회수한다. 테스트 카테고리에서는 낚시와 일반 위험 회피, 수정 연쇄 낙석, 심장수호자 공명 폭주, 성계 교차포화 미니게임 프리셋을 보상 없이 실행한다.
...
관리자 UI가 Inventory/SkillBook/Progress 내부 컬렉션이나 DB row를 직접 수정하지 않도록 `Player.adjustLevel`, `Inventory.clear`, `SkillBook.revoke`, `CareerProfile.setByAdmin`, `Resource.resetInteractionCooldown`, `markAllLocationsVisited`, `discoverAllCraftingRecipes`를 사용한다. 고빈도 플레이어 상태는 기존 Player dirty save 경계를 그대로 통과한다. 채팅 관리자 명령 `/레벨조정 <대상|me> <레벨>`도 같은 Player API를 사용하며 소수부는 경험치 진행률로 처리한다.
