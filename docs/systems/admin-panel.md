...
- 플레이어 작업: 순간이동, 레벨·스탯·직업·자원 설정, 아이템 지급·삭제, 보유 스킬 레벨 설정·삭제, 상태이상 관리, 부활, 전체 지도와 제작법 발견 처리, 선택한 온라인 플레이어 알림을 카테고리별로 실행한다.
...
4. 처리 후 호환용 `adminPanelResult`, 갱신된 플레이어 목록과 대상 상세가 요청 소켓에만 돌아온다. 화면 피드백은 별도 성공/실패 카드를 만들지 않고 기존 `notification`으로 자동 소멸한다.
...
`client/src/components/dialog`의 `Dialog`는 portal, Escape/배경 닫기, 포커스 복원과 body 스크롤 잠금을 제공한다. `FormDialog`는 필드 정의 배열로 text, number, select, textarea, checkbox 입력과 비동기 제출 상태를 만들며 필수 select는 첫 option을 기본값으로 선택한다. 이후 관리자 작업이나 화면 오버레이 입력은 별도 모달을 만들기보다 이 API를 우선 재사용한다.
