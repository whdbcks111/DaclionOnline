# 관리자 패널

권한 10 이상의 운영자가 햄버거 메뉴의 `관리자 페이지` 버튼으로 `/admin`을 새 탭에서 연다. 클라이언트에서 버튼을 감추는 것은 편의 기능이며, 모든 조회와 변경은 `server/src/modules/operations/adminPanel.ts`가 세션 권한을 다시 검사한다.

## 화면 구성

- 플레이어 관리: 전체 캐릭터를 조회하되 현재 접속자를 먼저 정렬한다. 닉네임·계정·UID 검색과 상세 상태, 인벤토리/장비, 스탯/스킬/칭호/상태이상 및 프레임/감정표현 검사를 제공한다.
- 플레이어 작업: 순간이동, 카르마·레벨·스탯·직업·자원 설정, 아이템 지급·삭제, 보유 스킬 레벨 설정·삭제, 칭호 부여·삭제, 꾸미기 지급·삭제, 우편 발송·삭제, 상태이상 관리, 부활, 전체 지도와 제작법 발견 처리, 선택한 온라인 플레이어 알림을 카테고리별로 실행한다. 칭호 부여 목록은 미보유 칭호, 삭제 목록은 보유 칭호만 표시하며 장착 칭호를 삭제하면 패시브도 즉시 정리된다. `꾸미기` 탭은 선택한 온라인·오프라인 유저의 미보유 프레임/30종 감정표현을 레벨·초월·Gold·낚시 뽑기 조건 없이 지급하고, 현재 보유 항목을 삭제한다. 감정표현 상세에는 실제 투명 이미지 미리보기를 표시한다. 프레임 삭제는 원형·채팅 장착을 함께 해제하며 자연 해금도 관리자 재지급 전까지 차단하고, 감정표현 삭제는 구매·뽑기·조건·관리자 지급 보유권을 모두 회수한다. `메시지` 탭의 우편 편집기는 여러 아이템과 Gold·칭호·스킬을 한 통에 함께 설정하며, 선택 상세에서 우편 상태를 검사하고 미수령 보상과 함께 우편을 삭제할 수 있다. 이미 수령한 보상은 우편 삭제로 역회수하지 않는다. 선택 상세에는 자연 감소가 반영된 현재 카르마와 단계가 표시되며 `카르마 설정`은 `Player.setKarma()`를 사용한다. `레벨 설정`은 값만 덮어쓰고, `레벨 조정`의 상승은 기존 스탯 분배를 정규화하지 않은 채 상승 레벨마다 모든 스탯 +1과 가용 포인트 +3만 더한다. 하락만 가용 포인트를 먼저 회수하고 부족분은 새 레벨의 자동 성장 최저값을 지키면서 기존 분배 비율에 가깝게 회수한다. 테스트 카테고리에서는 낚시와 일반 위험 회피, 수정 연쇄 낙석, 심장수호자 공명 폭주, 성계 교차포화 미니게임 프리셋을 보상 없이 실행하며 사람 확인 화면의 강제 실행·오탐 해제도 제공한다.
- 월드 관리: 장소별 몬스터 소환/리스폰과 자원 오브젝트 상호작용 쿨타임 초기화를 제공한다.
- 공지 발송: 모든 채널 기록에 남는 시스템 채팅 공지와 현재 접속자 전체에게 표시되는 화면 알림 공지를 분리해 제공한다. 알림 표시 시간은 1~60초, 내용은 최대 1000자로 서버가 다시 검증한다.
- 밸런스 분석: 검색 가능한 스킬·직업·장비/버프 아이템 선택창과 `전투 로테이션 프로파일`로 `Balance` 공개 API를 실행한다. 프로파일은 추천 장비, 평타+전체 성장 스킬, 공유 정신력/쿨다운, 동레벨 일반·보스 대상으로 평균 DPS·실제 처치 시각·최대 단발·첫 반격 전 폭딜과 양쪽 HP·피격 회피·확정 회피·보스 기술·단계별 생존시간을 표시한다.
- 작업 카테고리 탭은 라벨을 한 줄로 유지하며 패널 너비가 부족하면 가로 스크롤해 `인벤토리`, `스킬·효과` 같은 이름이 잘리지 않는다.
- 위치 편집기: 상단 `위치 편집기` 버튼으로 기존 `/admin/locations` 화면에 접근한다.

PC에서는 목록·상세·작업 패널을 다열로 배치한다. 상세 패널은 viewport 안에 고정된 내부 스크롤을 사용하고, 각 검사 영역도 펼쳤을 때 최대 높이와 독립 스크롤을 가져 긴 인벤토리·스킬 목록이 화면 밖으로 밀리지 않는다. 현재 상태는 자원별 색상의 전용 fill bar를 사용하며 진행 정보와 보유 현황은 한 카드에 묶어 높이를 줄인다. 좁은 화면에서는 플레이어 목록을 가로 스크롤로 축약하고 상세와 작업을 세로 배치하며, 공용 FormDialog는 모바일 하단 시트로 전환한다.

## 서버 흐름

1. `adminPanelRequestBootstrap`으로 아이템·밸런스 분석 대상 아이템·스킬·칭호·프레임·감정표현·직업·장소·몬스터·상태이상·스탯·미니게임 프리셋의 표시용 option 목록을 받는다.
2. `adminPanelRequestPlayers`와 `adminPanelRequestPlayer`로 목록 및 선택 캐릭터 snapshot을 받는다. 상세에는 현재 보관 중인 전체 우편의 stable ID·읽음·수령·만료 상태도 포함한다. 오프라인 캐릭터도 `fetchPlayerByUserId()`를 통해 공개 Player API로 로드할 수 있다.
3. `adminPanelExecute`는 action union과 입력값을 서버에서 다시 검증하고 도메인 목적형 API를 호출한다. 공지는 `broadcastBotMessageAll/broadcastNotification`, 개별 알림은 온라인 여부를 확인한 뒤 `sendNotificationToUser`를 사용한다.
4. 처리 후 호환용 `adminPanelResult`, 갱신된 플레이어 목록과 대상 상세가 요청 소켓에만 돌아온다. 일반 작업 피드백은 기존 `notification`으로 자동 소멸하며, 긴 밸런스 보고서만 `details`를 공용 Dialog로 연다. 전투 로테이션 보고서는 공격자와 피격자 양방향 회피, 90% 회피 요구치, 관통 적용 뒤 유효 방어, 첫 반격 전 처치와 처치 전 예상 피격·남은 생명력을 함께 표시한다.

관리자 UI가 Inventory/SkillBook/TitleBook/Progress 내부 컬렉션이나 DB row를 직접 수정하지 않도록 `Player.adjustLevel`, `Inventory.clear`, `SkillBook.revoke`, `TitleBook.grant/revoke`, `CareerProfile.setByAdmin`, `Resource.resetInteractionCooldown`, `markAllLocationsVisited`, `discoverAllCraftingRecipes`를 사용한다. 고빈도 플레이어 상태는 기존 Player dirty save 경계를 그대로 통과한다. 채팅 관리자 명령 `/레벨조정 <대상|me> <레벨>`, `/칭호부여 <대상|me> <칭호>`, `/칭호삭제 <대상|me> <칭호>`도 같은 Player API를 사용한다.

채팅 관리자 명령 `/우편발송 <유저ID|me|online|all> <아이템ID> <수량> [제목]`은 온라인 Player 객체나 우편 DB row를 직접 수정하지 않는다. `me`, `online`, `all`은 정확한 영문 소문자만 허용하고 대상 자동완성은 `me` → `online` → `all` → 중복 제거한 온라인 Player ID 순서다. 단일 대상은 `sendSystemMail()`, 실행 시점 온라인 snapshot은 ID를 다시 정렬·중복 제거하는 `sendSystemMailToRecipients()`, 오프라인을 포함해 캐릭터가 생성된 `Player` 행 전체는 `sendSystemMailToAllPlayers()`로 발송한다. `all`은 `User` 계정 전체를 뜻하지 않는다.

대량 우편은 입력 payload를 한 번만 정규화하고 수신자 100명씩 `createMany`하되 모든 청크를 하나의 interactive transaction 안에서 처리한다. 대상 누락이나 어느 청크의 실패든 전체를 rollback하며 0명은 우편을 만들지 않는 정상 no-op이다. 관리자 발송 payload에는 `sourceKey`가 없어 같은 명령을 반복하면 새 우편이 생기고, 성공·실패·0명과 최종 수신자 수는 실행한 관리자에게만 표시한다.

## 공용 다이얼로그

`client/src/components/dialog`의 `Dialog`는 portal, Escape/배경 닫기, 포커스 복원과 body 스크롤 잠금을 제공한다. `FormDialog`는 기본 필드와 복합 우편 보상 필드, 비동기 제출 상태를 만든다. `RewardBundleEditor`는 검색 가능한 선택기를 반복 추가해 Gold와 아이템·칭호·스킬 목록을 구성한다. 모든 select는 label·내부 코드·설명을 검색할 수 있는 공용 `SearchableSelect`를 사용한다. 선택 목록도 `document.body` portal로 분리해 다이얼로그 본문과 footer에 잘리지 않으며, PC에서는 선택 버튼 위·아래의 viewport 여유 공간을 계산해 더 넓은 방향으로 열고 모바일에서는 화면 안쪽 하단 목록으로 표시한다. 이후 관리자 작업이나 화면 오버레이 입력은 별도 모달을 만들기보다 이 API를 우선 재사용한다.
