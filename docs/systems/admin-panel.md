...
- 플레이어 작업: 순간이동, 레벨·스탯·직업·자원 설정, 아이템 지급·삭제, 보유 스킬 레벨 설정·삭제, 상태이상 관리, 부활, 전체 지도와 제작법 발견 처리, 선택한 온라인 플레이어 알림을 카테고리별로 실행한다. `레벨 설정`은 값만 덮어쓰고, `레벨 조정`은 레벨 차이마다 모든 스탯 +1과 가용 포인트 +3인 실제 성장 규칙을 함께 증감한다. 하락 시 가용 포인트를 먼저 회수하고 부족분은 새 레벨의 자동 성장 최저값을 지키면서 기존 분배 비율에 가깝게 회수한다. 테스트 카테고리에서는 낚시와 일반 위험 회피, 수정 연쇄 낙석, 심장수호자 공명 폭주, 성계 교차포화 미니게임 프리셋을 보상 없이 실행한다.
