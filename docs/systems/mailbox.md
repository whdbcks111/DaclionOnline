...
권한 10 관리자는 `/우편발송 <유저ID|me> <아이템ID> <수량> [제목]`으로 온라인 여부와 관계없이 존재하는 Player ID에 마스터 아이템을 보낼 수 있다. 대상 자동완성은 `me`와 현재 온라인 플레이어를, 아이템 자동완성은 등록된 마스터 ID와 표시명을 제공한다. 제목을 생략하면 운영 기본 제목을 사용하고 본문은 서버가 정한 안전한 지급 안내문으로 고정한다.

수량은 완전한 양의 정수만 허용하고 운영 1회 상한 10,000개와 우편 수령 시 생성 가능한 Item 행 100개를 모두 지킨다. stackable 아이템은 `maxStack × 100`, non-stackable 아이템은 100개가 추가 상한이다. 명령은 실제 `Item.snapshot()`으로 내구도·인스턴스 metadata·영속 태그 경계를 보존한 뒤 `sendSystemMail()`만 호출한다. 운영자가 같은 명령을 반복한 것은 별도 지급 의도로 보므로 `sourceKey`를 만들지 않으며, 성공·실패 결과는 실행한 관리자에게만 표시한다.
