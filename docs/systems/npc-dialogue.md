...
루미나르 전직소의 전직관 세레나는 대장장이를 포함해 현재 수락 가능한 모든 1차 직업 시험을 선택지로 표시하고, 진행 중/보고 가능 전직 퀘스트를 우선 진입점으로 안내한다. 실제 직업 변경은 대화 raw flag가 아니라 퀘스트 보상과 `CareerProfile` 공개 API가 소유한다. 광장의 의뢰관리인 세나는 계정과 KST 날짜로 고정된 토벌·채광·채집·낚시 중 오늘의 일일 의뢰와 경험치·골드·보급품 보상을 안내한다.
