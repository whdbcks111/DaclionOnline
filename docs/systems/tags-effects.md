...
기본 공격력
  ...
  -> 피격자 레벨 기반 비례 감산·나눗셈 혼합 방어 (방어력 - 공격자 관통)
  -> 최종 대미지
```

유효 방어는 `max(0, 방어-관통)`이며 피격자 레벨의 `K(L)=100+2L+0.005L²`로 정규화한다. 원피해의 일부를 비율로 감산한 뒤 남은 피해를 다시 방어 비율로 나누므로, 치명타 피해를 올려도 방어의 완화 비율이 사라지지 않는다. 세부 공식은 [전투 시스템](combat-ai.md#방어관통-계산)을 참고한다.
