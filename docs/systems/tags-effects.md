...
기본 공격력
  ...
  -> 관통 후 피격자 레벨 기준 고정 방어 + 비율 방어
  -> 최종 대미지
```

유효 방어 `e = max(0, 방어-관통)`와 피격자 레벨의 K에서 `q = K/(K+e)`를 구한다. 고정 방어 `e×q`를 먼저 빼고 남은 피해에 비율 방어 `1-q`를 적용하며, 공격자 레벨의 별도 피해 배율은 없다. 세부 공식은 [전투 시스템](combat-ai.md#방어관통-계산)을 참고한다.
