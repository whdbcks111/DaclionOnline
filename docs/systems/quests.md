...
루미나르 개척촌 광장의 의뢰관리인 세나는 한국 표준시 자정마다 다시 열리는 성장 일일 의뢰를 제공한다. 수락 레벨에 따라 Lv.1~49/50~149/150~299/300~499/500+의 다섯 의뢰 중 하나를 선택하며 목표량은 각각 8/12/16/20/24체다. 수락 순간 레벨을 Quest metadata에 고정하고 그 레벨의 80~120% 범위에 있는 일반 몬스터만 처치 목표로 인정한다. 보스는 제외되며 캐릭터 레벨이 도중에 바뀌어도 대상 범위는 변하지 않는다.

완료 보고 시점의 `Player.maxExp` 50%를 경험치로 지급하고 `daily:commission-last-claim-day` Progress state에 KST 날짜를 저장해 다른 레벨 구간 의뢰로 하루 보상을 중복 수령하지 못하게 한다. 전날 미완료 의뢰는 세나와 다시 대화할 때 포기 상태로 정리되어 새 의뢰를 막지 않는다. 퀘스트 진행은 기존 `player_quests` dirty 저장을, 마지막 보상일은 `PlayerProgress` dirty 저장을 사용하며 완료 직후 Player aggregate 저장을 요청한다.
