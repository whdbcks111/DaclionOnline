...
역설기계고의 `기억 톱니의 순서`는 Lv.200부터 기억 톱니와 논리핵을 실제 제출해 중계소 기록을 복원한다. 완료 뒤 Lv.218부터 `설계자의 마지막 모순`이 열리며 시간강 거신 제압, 역설 고정자 파괴, 역설설계자 오르도 제압을 순서대로 추적한다. 보스와 고정자는 전용 태그/Resource ID로 판정하며, 둘 모두 기록보존관 이델에게 수락·보고한다. 보상은 경험치·골드·지역 소모품·인과율 방패와 광자창/역설반전 전승서다.

공허왕관 성채의 `빛이 닿지 않는 귀환표식`은 Lv.275부터 무광은과 별먹을 실제 제출해 왕의 명령과 분리된 귀환로를 복구한다. 완료 뒤 Lv.290부터 `왕 없는 왕관의 판결`이 열리며 무관성주 제압, 전용 태그를 가진 공허왕관 기둥 셋 파괴, 공허섭정 제압을 추적한다. 둘 모두 기록수호자 세린에게 수락·보고하며 경험치·골드·지역 회복약·방패와 공허걸음/왕관무효 전승서를 보상한다.

월식해구의 `달빛 아래 잠긴 정박지`는 Lv.310부터 월염수와 침은을 실제 제출해 관측선의 조류기관을 복구한다. 완료 뒤 Lv.325부터 `끝나지 않는 백야`가 열리며 월조 리바이어던 제압, 전용 태그를 가진 조류거울 셋 파괴, 백야대사제 제압을 추적한다. 둘 모두 조류항해사 미레나에게 수락·보고하며 경험치·골드·지역 영약·방패와 역조보법/월식선고 전승서를 보상한다.

역근수해의 `수해가 잊은 귀환로`는 Lv.345부터 기억호박과 태초수액을 실제 제출해 귀환로의 기억을 복원한다. 완료 뒤 Lv.360부터 `첫 박동과 마지막 망각`이 열리며 역근 포식수 제압, 전용 태그를 가진 심장씨앗 셋 파괴, 태초심장 아르보르 제압을 추적한다. 둘 모두 기억수호자 오르넬에게 수락·보고하며 경험치·골드·지역 영약·방패와 파근낙하/태초성역 전승서를 보상한다.

성운회랑의 `꺼진 유성등을 잇는 길`은 Lv.380부터 성운유리와 궤도편을 제출해 귀환 궤도를 복구하고, 완료 뒤 Lv.400 `사건지평 너머의 왕관`에서 낙성감시자와 성운제를 차례로 제압한다. 동결시계원의 `멈춘 분침을 움직이는 법`은 Lv.420부터 시빙정과 역행사를 제출하며, 완료 뒤 Lv.440 `내일을 돌려놓는 시계`에서 빙시계 파수장과 영시여왕을 제압한다. 종언성단의 `꺼진 별을 잇는 선`은 Lv.460부터 잔광편과 창세정을 제출하고, 완료 뒤 Lv.480 `마지막 별자리의 이름`에서 종성의 전령과 최후성좌를 제압한다. 각 연속 퀘스트는 권역 안전 거점의 항해사·시계지기·관측자에게 수락·보고하며 지역 장비·영약·경험치·골드를 보상한다.
