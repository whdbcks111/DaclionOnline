...
Lv.50 이후 지역에서는 직업별 고레벨 무기 `풍뢰강 검`, `뇌운 시위`, `밤유리 단검`, `성휘목 지팡이`를 낮은 확률로 획득한다. 각각 공격력+속도, 공격력+치명타율, 공격력+관통과 부패 적중 효과, 마법력+정신력 재생을 제공하며 `ItemData.balance`로 실제 전후 전투 지표를 비교할 수 있다. 초·중반 궁수 장비는 가벼운 활 공격력 4, 은빛그물 사냥활 11, 진혼 시위 31, 뇌운 시위 37, 태양사 장궁 84, 빙실 연궁 128, 해무 조류궁 129로 성장한다. Lv.20·28·50·75·120·200 기준 추천 활 로테이션은 동급 검을 든 궁수보다 최소 5% 앞서도록 회귀 검증한다.
...
완성 조제약은 조합식 ID, 음용·투척 방식, 서버 재생 정확도와 감각에서 확정한 품질 입력을 인스턴스 metadata로 보존한다. 사용 시에는 metadata의 피해량·회복량·지속시간·상태효과를 그대로 신뢰하지 않고 등록된 조합식과 결과 ItemData를 다시 대조해 효과를 재계산한다. 음용은 사용자 자신에게만, 투척은 현재 대상 중심으로 같은 장소의 생존 파티원 또는 공격 가능한 몬스터에게만 서버가 다시 판정하며 잘못되거나 변조된 약은 소비하지 않는다.
...
은빛그물 숲의 `forest_antidote`는 30초 해독 상태로 독·맹독·마비독을 제거하고 재적용을 막는다. `silverweb_hunter_bow`는 공격력 11·치명타율 2%·투사체 가속 8%를 제공하는 초반 성장 활이다.

유리모래 사막은 유리모래·태양갑각·전갈 독낭·신기루 수정·태양문양 파편을 사냥·채집 소재로 제공한다. 오아시스 대추야자와 그늘 물통은 배고픔/수분을 회복한다. 모래쐐기 장검, 태양실 활, 신기루 독아, 태양유리 지팡이, 태양거울 방패는 Lv.70~110 성장 장비이며 공격·관통·치명타·투사체 가속 또는 물리/마법 생존을 역할별로 보강한다. 태양유리 지팡이는 마법력 107을 제공하고 다음 단계 극광분광 지팡이의 140보다 낮은 성장 순서를 유지하면서 Lv.75 직업별 추천 무기 피해 편차를 완화한다. 무기는 화염·빛·저주 적중 효과처럼 별도 상태효과를 명시적으로 호출하며 물리 공격 상성에 장착 무기 태그를 합산하지 않는 원칙은 유지한다.
...
`스탯포인트 초기화권`은 시스템 우편·운영 보상으로 지급할 수 있는 완전 초기화 소모품이다. `빛바랜 스탯포인트 초기화권`은 각 스탯 10개·총 50개까지만 환급하는 열화형이며 일반 보물상자 내부 약 0.99% 가중치와 11개 낚시터의 희귀 보물표 내부 가중치 4로 드물게 나온다. 두 아이템 모두 자동 레벨업 상승분을 보존하고 실제 환급이 없으면 소모하지 않는다. 현재 아이콘은 전용 아트 제작 전까지 기존 두루마리 원본을 데이터 ID별 파일로 복제한 fallback이다.
