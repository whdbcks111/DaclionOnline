...
월식해구와 백야성소는 월염수·월식비늘·침은·밤진주·해구섬유·조류인장을 사냥·채굴 소재로 제공한다. 월식 해초말이와 조류심장 영약이 장거리 탐색을 지원하고, 침은 파도검·월조류 장궁·밤진주 잠행도·월식 예언봉·백야 조류방패가 Lv.310~345 역할 장비를 구성한다. 두 보스의 전승서는 역조보법과 월식선고를 낮은 확률로 제공하며 전용 아트 전에는 존재하는 동종 카테고리 fallback만 사용한다.
...
은빛그물 숲의 `forest_antidote`는 30초 해독 상태로 독·맹독·마비독을 제거하고 재적용을 막는다. `silverweb_hunter_bow`는 공격력 8·치명타율 2%·투사체 가속 8%를 제공하는 초반 성장 활이다. 전용 아트 제작 전에는 기존 활·포션·유기물 카테고리 아이콘을 명시적 fallback으로 사용한다.
...
Lv.500~1000 승천 변경의 10개 전초 상점은 권역 소재 하나를 매입하고 단층검·원환궁·경계송곳니·공명지팡이·심층방패와 성장 가방, 대용량 HP/MP 포션과 화살을 판매한다. 장비 수치는 50레벨 단위로 상승하고 군주 보스는 같은 권역 소재, 숨은 제단용 군주 인장과 낮은 확률의 양면 능력 장신구를 드롭한다. 권역 유물함은 4~8시간마다 소재 묶음 또는 역할 장비 하나를 지급하며 중량이 부족하면 `Player.receiveLoot()`가 현재 장소 바닥에 보존한다. 전용 아트 제작 전에는 존재하는 역할별 아이템 fallback을 명시적으로 사용한다.
