...
`ItemData.bound`는 캐릭터 귀속 아이템의 소유권 제한이다. 귀속 아이템은 인벤토리에는 정상 보관되지만 `/버리기`, `/소각`, 플레이어 거래 에스크로로 옮길 수 없다. 초월 보상 `초월자의 나침반`은 무게 0의 비장착 귀속 아티팩트이며, 효과의 권위는 아이템 callback이 아니라 영속 `ascension:rank`와 `Player.gainExp()`가 가진다. 함께 지급하는 `초월자의 귀환 배낭`은 Lv.2000 이하 장착 조건과 최대 중량 +1,000kg을 제공해 환생 직후 보존된 인벤토리를 정리할 수 있게 한다.
...
Lv.50 이후 지역에서는 직업별 고레벨 무기 `풍뢰강 검`, `뇌운 시위`, `밤유리 단검`, `성휘목 지팡이`를 낮은 확률로 획득한다. 각각 공격력+속도, 공격력+치명타율, 공격력+관통과 부패 적중 효과, 마법력+정신력 재생을 제공하며 `ItemData.balance`로 실제 전후 전투 지표를 비교할 수 있다. 초·중반 궁수 장비는 가벼운 활 공격력 4, 은빛그물 사냥활 11, 진혼 시위 32, 뇌운 시위 37, 태양사 장궁 84, 빙실 연궁 128, 해무 조류궁 129로 성장한다. Lv.20·28·50·75·120·200 기준 추천 활 로테이션은 동급 검을 든 궁수보다 최소 3% 앞서도록 회귀 검증한다.
...
`/감정 <인벤토리 번호 또는 장착칸>`은 감각 50 이상에서 사용할 수 있다. 장착칸은 `손`, `다리`, `보조`, `장신구1` 같은 `EquipSlotType` 이름·별칭을 받는다. 감각 50에서는 설명·분류·수량·무게·가공된 속성을, 75에서는 내구도와 능력치 보정을, 100에서는 회복량·획득 스킬·탄약 방식·고유 적중 효과처럼 해석된 특수 효과를 추가로 공개한다. 사용 아이템은 `고유 효과 발동`으로 뭉뚱그리지 않고 음식의 배고픔·수분 회복량, 상태효과 이름·레벨·지속시간, 스킬 쿨다운 감소량, 스탯 환급 한도, 감정표현 해금, 미궁 유물과 연금 조제약의 실제 효과를 표시한다. 감각 75 이상의 장비 감정은 머리·몸통·다리·발·주무기·보조무기·장신구·가방 모두에 대해 현재 base·스탯·버프·스킬·장비의 add/multiply를 합친 최종 능력치를 읽기 전용으로 계산하고, 맨 마지막 `착용 시 변화` 구역에 비교 슬롯·기존 장비·변경된 능력치만 표시한다. 인벤토리 장비는 실제 장착과 동일하게 첫 빈칸, 모두 찼으면 마지막 점유 칸을 비교하며 장착 중인 장비는 조회한 정확한 칸을 비교한다. 같은 인스턴스는 변화 없음으로 표시하고, 파손·요구조건 미충족 장비도 가상 비교는 허용하되 실제 장착 시에는 원래 검사가 적용됨을 알린다. 비장비나 감각 75 미만에는 이 구역을 만들지 않는다. 같은 능력치 modifier는 `summarizeAttributeModifiers()`가 고정값을 합산하고 배율을 곱해 고정·비율 보정을 한 행에 표시한다. 내부 아이템 ID, raw 태그, metadata key/value는 권한과 관계없이 출력하지 않는다. 조회는 `Item.getInspectionSnapshot`, 자동완성은 `Inventory.getIndexedItems`와 `Equipment.getAllEquipped`를 사용한다.
...
바닥 아이템은 `Location.getDroppedItems()`의 복사본으로 표시하고 `pickupItem(index, count?)/pickupAllItems`로만 제거한다. `Location.addDroppedItem()`은 정의 ID·내구도·metadata delta·영속 태그가 같은 stackable 아이템을 공용 안전 상한까지 합치고 이때 묶음 전체의 5분 수명을 다시 시작한다. 서버 시각 기준 5분이 지난 묶음은 월드 tick과 모든 조회·줍기 직전 경계에서 제거되므로 만료 직전 UI를 이용해 획득할 수 없다. `/버리기 <슬롯> [개수|전체]`는 기본 1개를 버리고, 지정 수량이 보유량을 넘거나 `전체`이면 해당 인스턴스를 전부 버린다. `/소각 <슬롯> [개수|전체]`는 같은 수량 규칙을 사용하지만 바닥 아이템을 만들지 않고 영구 삭제한다. `/줍기 <번호> [개수]`는 개수를 생략하면 해당 스택 전체를, 지정하면 그 수량만 분리해 옮긴다. 전체 줍기는 유효한 모든 스택의 중량을 먼저 검사하므로 하나라도 받을 수 없는 경우 바닥 상태를 변경하지 않는다. 몬스터·파괴 가능한 자원 전리품은 `Player.receiveLoot()`로 먼저 인벤토리에 지급하고 중량이 부족하면 `Location.addDroppedItemData()`로 현재 장소 바닥에 생성한다.
...
- `resolveEquipTarget`: 실제 장착과 감정 미리보기가 공유하는 첫 빈칸·가득 찬 마지막 칸·명시 인덱스 선택 규칙을 불변 snapshot으로 반환한다.
- `previewItemAttributeChange`: 장비나 Attribute를 변경하지 않고 해당 슬롯 source만 후보 장비 modifier로 교체한 최종 능력치 변화 snapshot을 반환한다. 스택형 장비의 modifier도 수량과 무관하게 한 번만 적용한다.
...
아이템 조건은 `Item.requirements` 공개 snapshot과 `Player.getItemRequirementDeniedReason()`으로 검사한다. 최소 레벨·선택적 최대 레벨·스탯 조건을 같은 경계에서 확인한다. 감정 화면에는 레벨·스탯과 상점품/보물 완화/단조품 출처를 표시한다. 승천 유물함 장비는 같은 권역 상점품보다 완화된 약 50% 레벨·12% 핵심 스탯 조건을 사용해 희귀 획득의 조기 사용 보상을 유지한다.
...
`스탯포인트 초기화권`은 시스템 우편·운영 보상으로 지급할 수 있는 완전 초기화 소모품이며 이번 확장에서 획득처나 동작을 바꾸지 않는다. 반복 획득용 부분 환급권은 자동 레벨업 상승분을 보존한 채 `빛바랜`(스탯별 10·총 50), `정련된`(스탯별 25·총 100), `복원된`(스탯별 50·총 200) 순으로 직접 분배분을 균등하게 되돌린다. 빛바랜 권표는 일반 보물상자와 모든 낚시 희귀 보물표에, 정련된 권표는 보물 확률 2% 이상 낚시터와 Lv.400 이상 보스 2.5% 드롭 및 고레벨 제작에, 복원된 권표는 보물 확률 2.8% 이상 낚시터와 Lv.800 이상 보스 0.8% 드롭 및 최상위 제작에 들어간다. 실제 환급이 없거나 환급 뒤 장착 요구치를 잃으면 소모하지 않는다.
