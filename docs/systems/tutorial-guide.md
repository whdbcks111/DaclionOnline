...
낚시·광질·사냥 체험 단계에 들어가면 안내 버튼의 목적지인 물빛 연못·피버릭 갱도 입구·바람결 초원 들머리만 튜토리얼 권한으로 방문 처리한다. 이는 일반 월드의 잠금 조건을 해제하지 않고 자동이동 검색에 필요한 방문 기록만 추가한다. 이전 버전에서 이미 해당 체험 단계에 저장된 플레이어도 재접속 복원 시 같은 처리를 받아 버튼을 사용할 수 있다.
