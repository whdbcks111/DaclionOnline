...
일반 조작 명령은 `showCommandUse`를 생략하면 입력 메시지도 본인에게만 표시한다. 결과가 본인 전용인 줍기·버리기·장착·장착 해제는 입력 역시 비공개이므로 다른 사용자의 채팅에는 명령문만 단독으로 남지 않는다. 공개 추첨용 `/랜덤`처럼 입력과 결과가 모두 공개되어야 하는 명령만 `showCommandUse: 'show'`를 명시하고, 스킬·버튼처럼 별도 연출 메시지가 있는 명령은 `hide`를 사용한다.
