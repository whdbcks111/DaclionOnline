...
| 진행 | `/순위 [카테고리]` (`rank`, `ranking`) | 기본 레벨 또는 골드·스탯·능력치 순위. 상위 20명과 목록 밖 본인 순위를 고정 열 너비로 표시 |
...
| 위치 | `/위치` (`where`, `loc`, `l`, `m`) | 공개 모드에서만 채널에 공유하고 기본적으로 비공개인 통합 오브젝트·플레이어·드롭·1부터 시작하는 연결 순번 표시; 사망·파괴 오브젝트는 progress 대신 붉은 상태 표시 |
