...
권한·필수 인자 검증을 통과한 명령은 `subscribeCommandExecutions()`에 canonical 이름으로 전달된다. 첫 모험 튜토리얼은 안내 카드에 실행 버튼을 제공하지 않고 사용자가 직접 입력한 `/정식명령`과 슬래시 없는 별칭을 같은 학습 행동으로 처리한다. 도메인 성공 자체를 통계·퀘스트로 셀 때는 이 실행 이벤트가 아니라 실제 모델 결과의 GameEvent를 사용한다.
