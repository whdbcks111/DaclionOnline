...
`Forging`은 일반 제작법과 달리 하나의 고정 마스터 아이템을 그대로 복제하지 않는다. 장검·도끼·단검·방패·곡괭이 `ForgeForm`과 철·금·루비·에메랄드·다이아몬드 `ForgeMaterial`을 조합하고, 리듬 단조 정확도로 효율을 계산한다. 형태와 재료는 기본 공격/방어 수치·내구도·속성 태그를 결정하며, 균형·날 선·묵직한·정밀한 trait 하나가 보조 능력치를 바꾼다. 완성품 이름은 정확도/trait가 고른 판타지 접두어 어휘군, 실제 재료명, 형태별 RPG 명칭 후보를 조합한다. 후보 선택만 서버 난수로 달라지고 완벽한 품질은 형태 고유 접두어를 사용한다. 예를 들어 정확도 100%의 다이아몬드 도끼는 `익스클리프 다이아몬드 액스`가 된다.
