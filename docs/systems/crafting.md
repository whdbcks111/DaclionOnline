...
Lv.20부터 은맥 광부 보급소의 마도 대장장이 로안에게 `불꽃 없는 제련법` 퀘스트를 받을 수 있다. 광맥 8개를 파괴해 보고하면 전투 메인/서브 슬롯과 독립된 생산 전문 직업 `대장장이`와 대장장이의 담금질·마력 제련·금속 단조 스킬을 얻는다. 마력 제련은 별도 용광로·연료·대기열 없이 정신력을 사용해 인벤토리에서 가장 앞선 원광을 레벨당 일괄 처리한다.

`/단조 <형태> <재료>`는 대장장이 전문 직업 플래그 또는 `금속 단조` 스킬 중 하나를 보유하면 사용할 수 있다. 필요한 제련 소재를 확인하고 `forge_rhythm` 세션을 시작하며, 실패하면 선택 소재가 파손되고 성공하면 서버가 검증한 0~1 score를 장비 생성기에 직접 전달한다. 성공 결과는 소재와 완성품을 `Inventory.replaceSelectedItems()`로 한 번에 교환하고 단조 이벤트·경험치·완성품 정보를 보낸다. 클라이언트 메시지에서 정확도 문자열을 다시 파싱하지 않는다.
