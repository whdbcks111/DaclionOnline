...
`/단조 <형태> <재료>`는 필요한 제련 소재를 확인하고 `forge_rhythm` 세션을 시작한다. 실패하면 선택 소재가 파손되고, 성공하면 서버가 검증한 0~1 score를 장비 생성기에 직접 전달한다. 성공 결과는 소재와 완성품을 `Inventory.replaceSelectedItems()`로 한 번에 교환하고 단조 이벤트·경험치·완성품 정보를 보낸다. 클라이언트 메시지에서 정확도 문자열을 다시 파싱하지 않는다.
