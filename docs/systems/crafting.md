...
Lv.20부터 은맥 광부 보급소의 마도 대장장이 로안에게 `불꽃 없는 제련법` 퀘스트를 받을 수 있다. 광맥 8개를 파괴해 보고하면 현재 사용할 수 있는 메인 또는 서브 직업 슬롯에 정식 1차 직업 `대장장이`가 배정되고 대장장이의 담금질·결 파쇄·마력 제련·금속 단조 스킬을 얻는다. 다른 메인 직업을 이미 선택했다면 서브 슬롯이 열리는 Lv.50까지 기다려야 하며, 두 슬롯이 모두 차 있으면 전직할 수 없다. 마력 제련은 별도 용광로·연료·대기열 없이 정신력을 사용해 인벤토리에서 가장 앞선 원광을 `2 + 스킬 레벨 + floor(제련 정밀도 × 10)`개씩 처리한다.
