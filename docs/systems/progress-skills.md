...
현재 표준 이벤트 ID는 치명타, Entity 제압, Resource 파괴, 스킬 획득·시작·종료, 제작법 발견·아이템 제작, NPC 대화 시작·선택·종료, 상태효과 적용·갱신·제거다. 새 이벤트는 `GameEventIds`에 넣고 실제 상태가 확정되는 모델 API에서 `emitGameEvent()`를 호출한다.
