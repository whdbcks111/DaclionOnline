...
Lv.200 엘리트 직업 12개는 계승 패시브와 별도로 조합 전용 액티브 1종을 지급한다. 전사 계열은 추격·치명타·마력검, 궁수 계열은 중화기·야간 치명타·속성 제어, 암살자 계열은 집행·환영 투사체·맹독 마법, 마법사 계열은 보호막 돌진·고위력 별빛·저주 제어로 분화한다. 피해·정신력·쿨다운은 모두 `SkillData.balance`와 실제 발동 계산을 공유하며, 전용 아트 전에는 주계열 직업 아이콘을 fallback으로 쓴다.
