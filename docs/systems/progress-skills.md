...
성장 구간 보스는 이 경계를 그대로 사용해 초반의 `적갈기 도약`, `은실 포박`, `유충의 맹독`부터 `칼데라 분출`, `천둥 과부하`, `밤숲 가시`, `밤숲 재생`, `성역 심판`, `불사 진혼곡`까지 발동한다. 각 스킬은 피해 타입·상태이상·회피 가능 여부를 자체 정의하며 `skillPattern.randomOrder` 보스는 현재 발동 가능한 후보를 무작위 시작점부터 탐색한다. 별도 미니게임이 없는 보스도 실제 공용 스킬·랜덤 패턴·높은 보상으로 일반 몬스터와 구분한다.
