...
- 자동 획득: 첫 update와 관련 progress 변경 후 `autoAcquire.watchedProgress`만 다시 검사한다. Progress key가 없는 현재 상태 조건은 `alwaysEvaluate`를 명시한 정의만 0.5초 주기로 검사한다. 스탯 100 달성형 히든 패시브뿐 아니라 현재 직업 계보와 `unlockLevel`을 함께 확인하는 Lv.30·50 성장 기술도 이 경로를 사용한다.
...
- 전사: Lv.30 `파쇄 베기`, Lv.50 `철풍 돌파`.
- 궁수: Lv.30 `관통 화살`, Lv.50 `화살 폭우`.
- 암살자: Lv.30 `혈맥 절단`, Lv.50 `그림자 단검`.
- 마법사: Lv.30 `마력 창`, Lv.50 `홍염 파동`.
- 대장장이: Lv.30 `결함 간파`, Lv.50 `모루의 공명`·`담금질 방벽`. 감각이 높일 수 있는 치명타 피해를 확정 공격에 반영하고 높은 생존력과 생산 능력을 고려해 동레벨 전투 DPS는 순수 전투직보다 낮게 유지한다.

성장 기술의 실제 피해·관통·회피 불가·투사체 치명타·상태효과·보호막 callback은 설명과 `Balance`가 공유한다. Lv.50 프로파일 실측에서는 대장장이가 다른 직업보다 긴 보스 처치 시간을 갖는 대신 물리 생존 시간이 약 두 배이며, 전투직 네 계열은 같은 보스 기준 약 16~22초 범위다.
