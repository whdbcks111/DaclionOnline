...
성장 기술의 실제 피해·관통·회피 불가·투사체 치명타·상태효과·보호막 callback은 설명과 `Balance`가 공유한다. 회피 불가 투사체는 `ProjectileDataOverrides.unavoidable`이 비행 뒤 실제 `Entity.attack()`까지 보존된다. Lv.75/100/140/180 추천 장비·동레벨 보스 60초 회귀 프로파일은 다섯 1차 직업의 최고/최저 DPS 비율을 1.5배 이내로 고정하며, Lv.200의 순서 있는 20개 엘리트 조합도 같은 상한을 검사한다. 모든 프로파일은 평타 피해 비중을 최소 15% 이상 남긴다.
