...
현재 표준 이벤트 ID는 장소 도착, 치명타, 공격 적중·회피, 대상 지정, Entity 제압, Resource 파괴·성공 상호작용, 아이템 장착·성공 사용, 스탯 분배, 스킬 획득·시작·종료, 제작법 발견·아이템 제작, NPC 대화, 상태효과, 퀘스트, 직업 배정·엘리트 전직이다. 새 이벤트는 `GameEventIds`에 넣고 실제 상태가 확정되는 모델 API에서 `emitGameEvent()`를 호출한다. 명령 호출 자체는 성공 증거로 사용하지 않는다. 퀘스트 목표 추적은 [퀘스트 시스템](quests.md)을 참고한다.
