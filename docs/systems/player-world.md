...
- 지도 상세보기는 `worldMap` ChatNode 전용 SVG 컴포넌트다. PC는 휠 확대/축소와 drag, 모바일은 한 손가락 drag와 두 손가락 pinch/pan을 지원하며 blip hover/focus 카드에 이름·구역 유형·좌표를 표시한다. 장소 blip을 클릭하거나 터치하면 자동이동 확인 다이얼로그가 열리고, 방문한 다른 장소는 장소 ID 기반 `/자동이동`으로 바로 경로 탐색을 시작한다. 현재 위치와 인접 미방문 장소는 이유를 표시하되 실행하지 않는다.
