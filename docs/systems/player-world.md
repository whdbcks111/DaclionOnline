...
- Lv.200부터 성계균열의 성계 문지기 너머 역설기계고에 진입한다. 24개 장소는 입구의 고철벌 사냥 루프와 시간강 주조소·논리 기록원·기억 회랑의 세 갈래를 순환 연결하고, 질문 장치로 여는 숨은 원형 보관고와 두 보스 동선을 포함한다. Lv.220 시간강 거신은 회피 불가 속박을 섞고, Lv.235 역설설계자 오르도는 3개의 역설 고정자가 남아 있는 동안 받는 피해가 75% 감소한다.
...
- Lv.275부터 재왕의 왕좌 너머 공허왕관 성채에 진입한다. 27개 장소는 마른 해자 사냥 루프와 하층 안뜰의 성벽/정원, 외성문 뒤 주조실/기록고, 최상층의 관측소/법전고로 갈라졌다 다시 합쳐진다. Lv.290 무관성주 테오른은 공허창과 성벽 파단을 순서대로 사용하고, Lv.310 공허섭정 라시엘은 치유·보호·제어 위협을 계산해 세 기술을 무작위로 사용한다. 왕관 기둥 셋이 하나라도 남으면 라시엘이 받는 피해가 60% 줄어들며, 빈 왕좌의 서약을 풀면 hidden 금고와 첨탑 우회로가 열린다.
...
- Lv.345부터 백야성소 대제단 너머 역근수해·태초심장에 진입한다. 26개 장소는 하늘수관/부패수액 사냥 루프와 광휘뿌리/부패공동, 포자정원/기억호박 수로, 성수관/종자기록고와 광명/망각 천개로 갈라졌다 합쳐진다. Lv.360 역근 포식수는 낙하와 포자 숨결을 순서대로 사용하고, Lv.380 태초심장 아르보르는 치유·보호·제어 위협을 계산해 세 기술을 무작위로 사용한다. 심장씨앗 셋이 하나라도 남으면 아르보르가 받는 피해가 70% 줄어들며, 첫 기억의 제단을 풀면 hidden 기억호박 유물고 우회로가 열린다.
- Lv.380부터 태초심장 너머 성운회랑에 진입한다. 14개 장소는 성진 단구/혜성 주조소와 무음 궤도/중력 합류정의 두 단계 분기로 구성된다. Lv.400 낙성감시자 모르가는 중력 망치와 낙성을 순서대로 사용하고, Lv.420 성운제 아스테리온은 사건지평과 왕관섬광을 무작위로 사용한다.
- Lv.420부터 성운제의 왕좌 너머 동결시계원에 진입한다. 16개 장소는 입구 사냥 루프와 얼어붙은 분침/진자 빙하, 모래시계 묘역/뒤집힌 설원, 어제 회랑/내일 금고로 갈라졌다 합쳐진다. Lv.440 빙시계 파수장은 영시 절단과 진자뢰를 순서대로 사용하고, Lv.460 영시여왕 크로니아는 정지와 역행을 무작위로 사용한다.
- Lv.460부터 영시여왕의 왕좌 너머 종언성단에 진입한다. 17개 장소는 잿빛 궤도 사냥 루프와 재별무리/부서진 성좌, 침묵태양/맹세혜성, 창세/소멸 항로로 갈라졌다 최후지평에서 합쳐진다. Lv.480 종성의 전령 에녹은 신성과 침묵을 번갈아 사용하고, Lv.500 최후성좌 라스트라는 붕괴·창세·소멸을 무작위로 선고한다.
- Lv.500부터 최후성좌 너머 승천 변경에 진입한다. 천공묘역부터 기원경계까지 50레벨 폭의 10개 권역이 이어지며 각 권역은 전초거점, 외곽 순환로, 상·하층 회귀로, 갈라졌다 합쳐지는 나선 심층, 막다른 보물실과 가짜 종착지를 포함한 31개 장소를 가진다. 다음 권역 이행단층과 보스 심층이 서로 다른 갈래라 보스를 잡지 않아도 레벨 조건을 만족하면 탐험을 계속할 수 있다. 보스는 Lv.550~1000에 50레벨 간격으로 배치되고 치유·보호·제어 위협을 우선하는 고지능 AI와 두 개의 SkillData를 사용한다.
...
- `/위치`는 정보 공개 모드를 따르며 기본적으로 본인에게만 보이고 공개 모드에서만 현재 채널에 공유된다. 구역 위험도와 PVP 허용 여부를 먼저 표시하고 Monster와 Resource를 경계 없이 `[ 오브젝트 ]` 번호 목록으로 표시한다. PVP 가능 지역의 다른 생존 플레이어에는 `PVP 대상` 버튼을 표시한다. 사망·파괴된 오브젝트는 체력 progress 대신 붉은 `(사망)`·`(파괴됨)` 상태를 표시한다. 기본 리젠 주기가 5분을 초과하는 보스는 이름 옆에 리젠 주기를 표시하고, 처치된 동안에는 남은 시간을 표시한다. 등록된 상호작용 handler가 있는 살아 있는 오브젝트에는 `/상호작용 번호` 버튼을 붙이며, 나머지는 같은 명령을 실행해도 불가 알림을 보낸다.
...
현재 월드는 런타임 생성 권역을 포함해 총 602개 장소이며 다음 성장 동선을 제공한다. Lv.100 이후 권역은 연결과 성장 순서를 바꾸지 않은 채 권역 전체를 회전·이동해 동쪽 일변도의 진행선 대신 동서남북으로 굽는 2차원 대륙 형태로 배치한다. 홍염산지·유리모래 사막·서리잔향 설원·역설기계고·공허왕관·역근수해·동결시계원·종언성단에는 기존 관문을 우회하지 않고 같은 권역 본선으로 다시 합류하는 2개 장소짜리 대체 사냥 루프가 있다. 성장 관문 보스는 Lv.500까지 최대 30레벨 간격, 이후 Lv.1000까지 50레벨 간격으로 등장하고 같은 레벨 일반 몬스터보다 최소 5배 경험치와 훨씬 높은 생명력을 가진다. 모든 보스가 미니게임을 강제하지 않으며 일부는 복수 SkillData 무작위 패턴·상태이상·주변 오브젝트 기믹으로 차별화한다.
...
| 역근수해·태초심장 | Lv.345~380 최종 성역 | 광휘/부패 뿌리와 기억호박 수로 분기, Lv.360 역근 포식수, 첫 기억 제단과 숨은 유물고, 심장씨앗 셋을 먼저 파괴하는 Lv.380 태초심장 |
| 성운회랑 | Lv.380~420 성간 회랑 | 성진/혜성 및 무음궤도 분기, 혜철 성운맥, Lv.400 낙성감시자와 Lv.420 성운제 |
| 동결시계원 | Lv.420~460 시간 동결 정원 | 얼어붙은 분침/모래시계 묘역과 어제/내일 분기, 진자강 시빙맥, Lv.440 빙시계 파수장과 Lv.460 영시여왕 |
| 종언성단 | Lv.460~500 최종 성단 | 재별/침묵태양과 창세/소멸 항로 분기, 소멸금 성단맥, Lv.480 종성의 전령과 Lv.500 최후성좌 |
| 승천 변경 10권역 | Lv.500~1000 다중 순환 미궁 | 천공묘역부터 기원경계까지 권역별 31개 장소, 외곽·상하층·나선 순환로, 막다른 유물함·가짜 종착지·hidden 제단, 진행을 막지 않는 Lv.550~1000 선택형 보스 |
...
`sendLocationInfo()`는 현재 위치 좌표·위험도 라벨·PVP 허용 여부·인접 위치·동일 위치 플레이어·통합 `objects`의 생명력과 보호막, 현재 플레이어 기준 가능한 행동, NPC 표시 정보를 해당 사용자의 모든 소켓에 보낸다. 인접 위치는 `Location.getAvailableConnections(player)` 결과라 hidden은 제외되고 visible/locked 상태가 포함된다. NPC는 `Location.getNpcs()`와 `QuestBook.getNpcMarker()`의 가공된 이름·설명·퀘스트 표식만 전송한다. `Home.tsx`가 이를 `HudContext`에 저장하고 PlayerStatus/TargetStatus/Party/Location/Minimap HUD가 소비한다. TargetStatus HUD는 대상이 있을 때만 나타나는 독립 HUD이며 설정에서 표시·위치·크기를 조절하고 편집 모드에서는 미리보기를 표시한다. Location HUD는 서버가 보낸 `attack | target | interact` 행동만 버튼으로 만들므로 공격 불가 상자·질문 장치에는 상호작용만 표시하고, NPC 행에는 `/대화 번호` 버튼을 표시한다. PVP 가능 지역의 플레이어 행에는 `/대상지정p #고유번호` 버튼을 더한다. Minimap HUD는 세부 옵션을 켠 경우 visible 인접 지역만 이동 목록에 표시하고 `/이동 ID`를 숨김 실행한다.
