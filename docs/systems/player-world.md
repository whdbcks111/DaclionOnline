...
- `Player.earlyUpdate`: Entity 공통 갱신 뒤 `hungerDrain`과 `thirstDrain`을 초 단위로 적용한다. 사망·제압 중에는 감소하지 않고 두 자원 모두 0 아래로 내려가지 않는다.
...
- Attribute: maxLife, maxMentality, lifeRegen, mentalityRegen, hungerDrain, thirstDrain, maxThirsty, maxHungry, maxWeight, atk, magicForce, def, magicDef, armorPen, magicPen, speed, attackSpeed, critRate, critDmg. 두 재생 능력치의 기본값은 초당 1, 배고픔/수분 감소량은 각각 초당 0.01/0.02이며 `/상태창` 능력치 영역에 `/초` 단위로 표시된다.
- 19개 `AttributeType`은 모두 `attributes/{key}` 대표색 아이콘을 소유한다. 상태창은 자원 bar와 각 능력치를 `아이콘 + 이름 + 값` 한 행으로 표시해 긴 재생/감소량 이름도 겹치지 않으며, 이름 hover에는 계산 설명을 유지한다.
...
- 직접 공격과 회피의 결과 메시지·notification은 공격자와 피격 플레이어에게만 전송하며, 같은 장소에 있다는 이유만으로 주변 플레이어에게 전투 알림을 보내지 않는다. 대응이 필요한 몬스터 스킬의 시전 예고는 별도의 패턴 알림으로 유지한다.
...
- Projectile은 좌표가 없는 현재 월드 모델에 맞춰 지정된 비행 시간 뒤 같은 위치의 target을 공격하고 즉시 소멸한다. 영속 저장하지 않으며 `spawnProjectile` 또는 마스터 데이터 기반 `spawnProjectileFromData`로 생성한다. 생성 시 owner의 치명타 확률·치명타 피해를 투사체 능력치에 스냅샷으로 동기화하므로 활 기본 공격, 궁수 화살과 마법사 마법 투사체 모두 독립적으로 치명타를 판정한다. `attributeOverrides`를 지정하면 동기화된 값도 명시적으로 덮어쓸 수 있다.
...
- `LocationData.mapColor`는 선택적인 `#RRGGBB` 대표색이다. 방문한 장소의 대표색만 지도 노드 아래에서 주변으로 번지며, 가까운 장소 간 거리로 반경을 자동 조절하고 노이즈 변형·그라데이션으로 인접 색과 부드럽게 섞어 바이옴 영역을 만든다. 미방문 장소와 일반 지도에서 제외된 hidden 장소의 색은 노출하지 않는다.
...
- `/이동` 시간은 `max(1, distance / speed / 5)`초이고 0.5초 단위 coroutine 알림을 갱신한다. 대상은 정식 장소명 외에도 location ID, 공백·가운뎃점·하이픈을 생략한 이름, 현재 연결 중 유일한 부분 이름으로 찾을 수 있다. 양의 정수는 `getAvailableConnections()` 목록의 1부터 시작하는 순번으로 먼저 해석하므로 `go 1`, `mv 2`도 장소 이름의 숫자 포함 여부와 무관하게 일관되게 동작한다. 예: `/이동 초원3`, `/이동 meadow_3`.
...
현재 월드는 총 24개 장소이며 다음 성장 동선을 제공한다.
...
`sendPlayerStats()`는 본인의 현재 레벨, 자원, 공격 cooldown, 상태효과의 ID·아이콘·레벨·현재/최대 시간·계산 설명과 nullable 파티원 HP/MP snapshot을 보낸다. `sendLocationInfo()`는 현재 위치 좌표·인접 위치·동일 위치 플레이어·통합 `objects`를 해당 사용자의 모든 소켓에 보낸다. 인접 위치는 `Location.getAvailableConnections(player)` 결과라 hidden은 제외되고 visible/locked 상태가 포함된다. `Home.tsx`가 이를 `HudContext`에 저장하고 PlayerStatus/Party/Location/Minimap HUD가 소비한다. Location HUD는 각 살아 있는 오브젝트 행에 설정 가능한 `공격`/`대상` 버튼을 더하고 각각 기존 `/공격 번호`, `/대상지정 번호` 명령을 숨김 실행한다. Minimap HUD는 세부 옵션을 켠 경우 visible 인접 지역만 이동 목록에 표시하고 `/이동 ID`를 숨김 실행한다.
