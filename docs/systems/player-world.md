...
- `/위치`는 바닥 아이템도 개별 번호와 줍기 버튼으로 표시한다. 같은 인스턴스 상태의 stackable 드롭은 최대 스택까지 합쳐지며 `/버리기 번호 [개수|전체]`로 일부 또는 전부를 내려놓을 수 있다. 요청 수량이 보유량보다 크면 보유량 전부를 사용한다. `/줍기 번호 [개수]`는 개수를 생략하면 해당 드롭 스택 전체를, 지정하면 일부만 옮긴다. `/줍기 전체`는 중량이 허용할 때 모든 스택을 인벤토리로 옮긴다. 처치·파괴 전리품을 받을 중량이 부족하면 소멸하지 않고 현재 장소 바닥에 생성된다.
