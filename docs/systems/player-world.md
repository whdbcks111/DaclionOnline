...
승천 권역에는 성락 심해광산·적핵 폐광·봉헌 지하광산·태초맥 광산이 각각 4개 장소의 분기·심층·희귀맥 순환 동선을 이룬다. 광맥 리젠은 공용 3분 상한을 따르며 광산마다 희귀 원광 두 종이 전용 드롭이다.

압해 석호·먹물꿈 연못·수은 저수지·무언 기도샘·창세 조수대는 전초 거점에서 접근하는 중립 고레벨 낚시터다. 장소 ID는 낚시 테이블 key로 사용되어 어종 풀과 가중치를 독립적으로 선택한다.

Lv.500 이전에도 유리모래 오아시스부터 라그나벨 저문별 호수까지 다섯 단계의 낚시터가 배치된다. 같은 구간의 유리모래·상고 수정·시간강·침은·소멸금 광맥은 3% 희귀 원광과 대응 제련 소재를 제공해 Lv.600 희귀 광산 이전 생활 성장 공백을 메운다.
...
- Stat: strength, agility, vitality, sensibility, mentality. `StatType` 클래스형 enum의 modify 함수가 Entity를 받아 Attribute modifier를 적용하며, 각 타입은 상태창과 `/스탯분배`에 노출할 파생 능력치 목록도 소유한다. 따라서 감각의 제련 정밀도처럼 직접 수치 설명에서 빠지기 쉬운 파생값도 함께 표시된다.
- 체력 `p`의 최대 생명력 기여는 `10p + 0.006p²`, 방어력 기여는 `p + 0.01p²`다. 정신력 `p`의 마법 저항력 기여는 `0.5p + 0.01p²`다. 특정 레벨 경계 없이 자동 스탯과 직접 투자가 누적될수록 생존 수치가 가속되며, 방어 계열은 낮은 공격을 거르는 고정 방어와 큰 공격을 줄이는 비율 방어에 함께 쓰인다. 정신력의 최대 정신력 +5.25·마법력 +2·마법 관통력 +0.5·투사체 가속 +0.002는 선형 성장하므로, 동량 투자에서도 제곱 성장하는 마법 저항력이 레벨 격차가 큰 약한 공격을 자연스럽게 차단한다.
...
- 감각에서 얻는 치명타율은 `0.5 × (1 - e^(-감각/500))`다. 0 근처에서는 기존처럼 감각 1당 약 0.1%p지만 포인트가 높아질수록 증가량이 감소하며 감각 100/500/1000에서 각각 약 9.1%p/31.6%p/43.2%p, 최대 50%p에 점근한다. 치명타 피해는 감각 1당 +1%, 제련 정밀도는 감각 1당 +0.15%로 선형 성장한다. 최종 치명타율만 전투 판정에서 0~100%로 보정한다.
- `Entity.attack`의 optional `AttackOptions`는 해당 한 번의 `criticalRate/criticalDamage/consumeMainHandDurability/unavoidable/fixedDamage/triggerMainHandHitEffects/armorDurabilityDamageMode`만 override한다. `unavoidable`은 회피 판정을 생략하고 `fixedDamage`는 치명타·상성·방어·관통을 모두 생략해 입력 피해량을 그대로 적용한다. 주무기 적중 효과는 기본적으로 피해가 들어간 물리 직접 공격에서 실행한다. 방어구 내구도는 보호막 뒤 실제 생명력 피해가 있을 때 기본 `SINGLE` 확률·부위 가중치로 한 곳만 손상되고, 특수 공격이 `ALL`을 명시한 경우에만 장착 중인 네 방어 부위를 모두 한 번씩 손상시킨다.
...
- `physical` 피해는 방어력/물리 관통력, `magic` 피해는 마법 저항력/마법 관통력을 사용한다. 피격자 레벨 `L`에서 `K = 100 + 2L + 0.005L²`(Lv.200 이하), 이후 `K = 4L - 100`이며, 유효 방어 `e = max(0, 방어-관통)`의 남는 피해 비율은 `q = K/(K+e)`다. 같은 능력치가 `e×q`의 고정 방어와 `1-q`의 비율 방어를 제공하고 최종 피해는 `max(0, 원피해-eq)×q`다. 낮은 방어에서는 고정 감산이 중심이고 높은 방어에서는 고정값이 K에 점근하면서 비율 방어가 강해진다. 공격자와 피격자의 레벨 차이를 결과에 다시 곱하는 별도 피해 배율은 사용하지 않는다. `absolute`는 방어·관통만 생략하고 회피·치명타·속성 상성은 유지하므로 모든 계산을 생략하는 `fixedDamage`와 구분한다.
...
- Player의 접속 중 `autoAttackEnabled` 모드는 `/자동공격` 또는 HUD 설정에서 표시한 전투 퀵 버튼으로 토글한다. 자동공격 퀵 버튼은 신규 설정에서 기본 숨김이다. 활성화 상태에서 현재 타겟이 같은 장소의 유효한 공격 대상이고 공격 행동이 가능하며 cooldown이 0이면 `Player.performBasicAttack()`을 한 번 실행한다. 대상이 없거나 사망·이탈·상호작용 전용이면 조용히 대기하고 모드 자체는 유지한다.
...
- 모든 `MonsterData`는 감정용 `description`과 이미지 key를 가진다. 기존 월드·승천 권역 198종은 데이터 ID별 64×64 RGBA 전용 초상을 사용하고, 1차 확장 중인 균열 원형 76종은 동계열 fallback 초상을 명시적으로 재사용한다. `/위치`, 위치 HUD, 타게팅 HUD, `/몬스터정보`의 이름 앞에는 같은 공개 아이콘을 표시한다. `/몬스터정보 번호`는 감각 100 이상에서 현재 장소의 Monster만 조회하며 기본 설명·상태·가공된 속성을 공개한다. 감각 125에서는 전체 전투 능력치·기본 피해 타입·적중 상태효과를, 150에서는 드롭 확률·경험치·골드·스킬 패턴·장비를 추가로 공개한다. 내부 몬스터 ID와 raw 태그는 출력하지 않는다.
- Monster 사망 시 마지막 공격원의 `attackOwner`가 Player이면 드롭과 골드는 처치자에게 지급한다. 경험치는 처치자가 파티에 있으면 같은 장소의 생존 파티원에게도 지급하며 최고 레벨과의 차이에 100%/50%/20%/10% 감쇠를 적용한다. 보스가 아닌 일반 몬스터가 참여자 본인 레벨의 60% 미만이면 개인별로 최대 절반까지 추가 감쇠하며 골드·드롭·퀘스트 진행에는 적용하지 않는다. 처치 경험치는 지급 직후 다음 Player 사망 판정까지 획득분만 보호해, 같은 프레임에 처치자도 사망하더라도 그 경험치를 사망 손실로 다시 차감하지 않는다. 상세 규칙은 [파티 시스템](party.md)을 참고한다.
...
- Lv.200부터 성계균열의 성계 문지기 너머 카이로스 공방도시에 진입한다. 24개 장소는 입구의 고철벌 사냥 루프와 시간강 주조소·논리 기록원·기억 회랑의 세 갈래를 순환 연결하고, 질문 장치로 여는 숨은 원형 보관고와 두 보스 동선을 포함한다. Lv.220 시간강 거신은 회피 불가 속박을 섞고, Lv.235 역설설계자 오르도는 3개의 역설 고정자가 남아 있는 동안 받는 피해가 75% 감소한다.
- Lv.235부터 끝없는 관측소 너머 아셴바흐 심연에 진입한다. 25개 장소는 망자 골짜기·공허어금니/백골바람 분기, 흑염 회랑 순환로, 카르모르 성 내성의 병영/성벽 분기로 구성된다. 회색불길 길잡이 타렌의 재료 복구 의뢰를 마치면 세 단계 보스 연속 토벌 의뢰가 열린다. Lv.248 세 아귀 문지기, 치유·보호·제어 위협을 우선하는 Lv.260 흑염대장 모르칸, 도발에 거의 흔들리지 않는 Lv.275 재왕 벨카르가 단계 관문을 이룬다. 봉인 예배당의 질문을 풀면 hidden 유산고와 7~10시간 유물함이 열린다.
- Lv.275부터 재왕의 왕좌 너머 벨카인 요새에 진입한다. 27개 장소는 마른 해자 사냥 루프와 하층 안뜰의 성벽/정원, 외성문 뒤 주조실/기록고, 최상층의 관측소/법전고로 갈라졌다 다시 합쳐진다. Lv.290 무관성주 테오른은 공허창과 성벽 파단을 순서대로 사용하고, Lv.310 공허섭정 라시엘은 치유·보호·제어 위협을 계산해 세 기술을 무작위로 사용한다. 왕관 기둥 셋이 하나라도 남으면 라시엘이 받는 피해가 60% 줄어들며, 빈 왕좌의 서약을 풀면 hidden 금고와 첨탑 우회로가 열린다.
- Lv.310부터 벨카인의 빈 왕좌 아래 침수 승강정으로 내려가 루나리스 해구에 진입한다. 24개 장소는 밝은 산호초/침몰 선단, 해구숲/검은 역조, 에일린 대성당의 성가회랑/조류기관실로 갈라졌다 합쳐진다. Lv.325 월조 리바이어던은 월조 해일과 해압 분쇄를 순서대로 사용하고, Lv.345 백야대사제 세르미아는 치유·보호·제어 위협을 계산해 백야와 월식 기술을 무작위로 사용한다. 조류거울 셋이 하나라도 남으면 세르미아가 받는 피해가 65% 줄어들며, 월식 조류제단의 답을 풀면 hidden 침수보물고 우회로가 열린다.
- Lv.345부터 에일린 대성당 대제단 너머 카미하라 숲·에오나의 심장에 진입한다. 26개 장소는 하늘수관/부패수액 사냥 루프와 광휘뿌리/부패공동, 포자정원/기억호박 수로, 성수관/종자기록고와 광명/망각 천개로 갈라졌다 합쳐진다. Lv.360 역근 포식수는 낙하와 포자 숨결을 순서대로 사용하고, Lv.380 에오나의 심장 아르보르는 치유·보호·제어 위협을 계산해 세 기술을 무작위로 사용한다. 심장씨앗 셋이 하나라도 남으면 아르보르가 받는 피해가 70% 줄어들며, 첫 기억의 제단을 풀면 hidden 기억호박 유물고 우회로가 열린다.
- Lv.380부터 에오나의 심장 너머 아스트라 회랑에 진입한다. 14개 장소는 성진 단구/혜성 주조소와 무음 궤도/중력 합류정의 두 단계 분기로 구성된다. Lv.400 낙성감시자 모르가는 중력 망치와 낙성을 순서대로 사용하고, Lv.420 성운제 아스테리온은 사건지평과 왕관섬광을 무작위로 사용한다.
...
- 루미나르 광장과 황혼왕릉부터 라그나벨까지 12개 지역 안전 거점에는 공격할 수 없는 `공간 중계핵`이 하나씩 있다. 루미나르는 기본 해금이며, 나머지는 해당 지역의 첫 복구·보급 퀘스트를 완료한 뒤 5만~800만 Gold를 내야 영구 해금된다. `/중계소 [목적지]`는 현재 중계핵에서 해금한 거점만 선택하며 목적지 단계에 따라 500~8만 Gold, 루미나르 귀환은 1천 Gold를 사용료로 차감하고 즉시 이동한다. `/거주점설정`은 현재 해금 중계소를 추가 비용 없이 부활 거점으로 저장한다. 해금 FLAG와 거주점 STATE는 일반 Progress와 함께 30초·unload·종료 flush되며 초월 뒤에도 보존된다.
...
- `/위치`는 바닥 아이템도 개별 번호와 줍기 버튼으로 표시한다. 같은 인스턴스 상태의 stackable 드롭은 DB 안전 상한까지 합쳐져 일반 플레이에서는 하나의 묶음으로 보이며 새 수량이 합쳐지면 묶음 전체의 수명이 갱신된다. 바닥 아이템은 서버 시각 기준 5분 뒤 월드 tick과 조회·줍기 직전 경계에서 제거된다. `/버리기 번호 [개수|전체]`로 일부 또는 전부를 내려놓을 수 있고 요청 수량이 보유량보다 크면 보유량 전부를 사용한다. `/줍기 번호 [개수]`는 개수를 생략하면 해당 드롭 스택 전체를, 지정하면 일부만 옮긴다. `/줍기 전체`는 중량이 허용할 때 모든 유효 스택을 인벤토리로 옮긴다. 처치·파괴 전리품을 받을 중량이 부족하면 현재 장소 바닥에 생성되지만 같은 5분 수명을 따른다.
...
- 지도 상세보기는 `worldMap` ChatNode 전용 SVG 컴포넌트다. 방문한 안전·중립·적대 지역 노드와 랜드마크 테두리를 각각 파랑·청록·적색으로 표시하고, `location:boss_room`은 위험도보다 높은 우선순위의 금색으로 표시해 상세 카드에도 보스 구역을 명시한다. 인접 미방문 장소는 위험도와 보스 여부를 탐색 전에 공개하지 않고 회색으로 유지한다. PC는 휠 확대/축소와 drag, 모바일은 한 손가락 drag와 두 손가락 pinch/pan을 지원하며 blip hover/focus 카드에 이름·구역 유형·좌표를 표시한다. PC 마우스는 누르는 즉시 포인터를 캡처하지 않고 6px 이상 움직여 실제 drag가 시작된 뒤 캡처하므로 짧은 클릭이 지도 전체로 재지정되지 않는다. 장소 blip을 클릭하거나 터치하면 자동이동 확인 다이얼로그가 열리고, 방문한 다른 장소는 장소 ID 기반 `/자동이동`으로 바로 경로 탐색을 시작한다. 현재 위치와 인접 미방문 장소는 이유를 표시하되 실행하지 않는다.
...
현재 월드는 런타임 생성 권역을 포함해 총 632개 정적 장소이며 다음 성장 동선을 제공한다. Lv.100 이후 권역은 연결과 성장 순서를 바꾸지 않은 채 권역 전체를 회전·이동해 동쪽 일변도의 진행선 대신 동서남북으로 굽는 2차원 대륙 형태로 배치한다. 홍염산지·유리모래 사막·서리잔향 설원·카이로스 공방도시·벨카인·카미하라 숲·에버프로스트 정원·라그나벨 성단에는 기존 관문을 우회하지 않고 같은 권역 본선으로 다시 합류하는 2개 장소짜리 대체 사냥 루프가 있다. 성장 관문 보스는 Lv.500까지 최대 30레벨 간격, 이후 Lv.1000까지 50레벨 간격으로 등장하고 같은 레벨 일반 몬스터보다 최소 5배 경험치와 훨씬 높은 생명력을 가진다. 초월 후 Lv.1000에 재도달한 플레이어는 아르케 직전 경계에서 잔재의 확인을 받아 보스방을 우회하고, 8개 장소로 이루어진 역지옥문 상위차원 원정망에 진입한다. 모든 보스가 미니게임을 강제하지 않으며 일부는 복수 SkillData 무작위 패턴·상태이상·주변 오브젝트 기믹으로 차별화한다.

`location:instance_dungeon` 장소는 원정마다 메모리에서 생성되고 폐기되는 별도 공간이다. 정적 `LocationData` 캐시, 관리자 장소 JSON, 지도와 영구 방문 기록에는 포함하지 않는다. Lv.10~960의 19개 균열 게이트는 10초 현장 합류 뒤 각 네 개의 방을 만들며, 모든 적을 쓰러뜨린 방만 다음 연결이 열린다. 일반 방의 몬스터는 살아 있는 참가자에게 순환 배정되어 둘 이상이면 전선을 나누지만 혼자면 무리 전체의 공격을 받는다. 구체적인 제한시간·귀환·세계관은 [차원 균열·인스턴스 던전](instance-dungeons.md)에 기록한다.
...
| 황혼왕릉 | Lv.28~58 선택 동선 | 마지막 등불 야영지, 백골/기사묘 두 갈래, Lv.45 해골왕과 Lv.58 기사왕, 질문 석문 뒤 4~6시간 유물함; 기사왕 경로는 Lv.50부터 천둥마루에 합류 |
...
| 카이로스 공방도시 | Lv.196~235 분기형 고대 자동도시 | 시간강·광자·논리·균열 소재, 기억 톱니 퍼즐, Lv.220 시간강 거신, 고정자 3개를 먼저 파괴하는 Lv.235 역설설계자 |
| 아셴바흐 심연·카르모르 성 | Lv.238~275 분기형 심연 왕국 | 망자 골짜기·흑염 회랑·카르모르 성 분기, Lv.248 세 아귀 문지기, Lv.260 흑염대장, 봉인 질문문과 숨은 유산고, Lv.275 재왕 |
| 벨카인 요새 | Lv.275~310 다층 왕성 | 성벽/정원과 주조실/기록고 분기, Lv.290 무관성주, 빈 왕좌 서약과 숨은 금고, 기둥 셋을 먼저 파괴하는 Lv.310 공허섭정 |
| 루나리스 해구·에일린 대성당 | Lv.310~345 침수 성역 | 산호초/침몰선과 해구숲/역조 분기, Lv.325 월조 리바이어던, 조류제단과 숨은 보물고, 거울 셋을 먼저 파괴하는 Lv.345 백야대사제 |
| 카미하라 숲·에오나의 심장 | Lv.345~380 최종 성역 | 광휘/부패 뿌리와 기억호박 수로 분기, Lv.360 역근 포식수, 첫 기억 제단과 숨은 유물고, 심장씨앗 셋을 먼저 파괴하는 Lv.380 에오나의 심장 |
| 아스트라 회랑 | Lv.380~420 성간 회랑 | 성진/혜성 및 무음궤도 분기, 혜철 성운맥, Lv.400 낙성감시자와 Lv.420 성운제 |
| 에버프로스트 정원 | Lv.420~460 시간 동결 정원 | 얼어붙은 분침/모래시계 묘역과 어제/내일 분기, 진자강 시빙맥, Lv.440 빙시계 파수장과 Lv.460 영시여왕 |
| 라그나벨 성단 | Lv.460~500 최종 성단 | 재별/저문별 호수와 창세/소멸 항로 분기, 소멸금 성단맥, Lv.480 종성의 전령과 Lv.500 최후성좌 |
| 승천 변경 10권역 | Lv.500~1000 다중 순환 미궁 | 세라핀 하늘묘지부터 아르케 끝자락까지 권역별 31개 장소, 외곽·상하층·나선 순환로, 막다른 유물함·가짜 종착지·hidden 제단, 진행을 막지 않는 Lv.550~1000 선택형 보스 |

고블린 정의와 배치는 제거됐다. 지역 관문은 기존 조건과 `level_500/550/600/650/700/750/800/850/900/950` condition으로 잠긴다. 관리자가 높은 지역으로 순간이동한 경우 낮은 지역으로 되돌아오는 일반 역방향 연결은 열어 두되, 같은 상위 관문에 합류하는 모든 진입로에는 동일한 레벨 조건을 적용해 자동이동 우회를 막는다. 일반 광석부터 소멸금까지의 광맥은 성장 구간에 맞춘 경도 20~820을 가지며, 곡괭이 없이도 공격하거나 마법을 사용할 수 있지만 곡괭이·버프·채굴 스킬의 채굴력이 경도를 상쇄해 훨씬 효율적으로 파괴한다. 몬스터 정의에는 불·물·얼음·자연·독·전기·돌·어둠·빛·언데드·신성·벌레·금속·땅 속성이 모두 실제 배치되어 `/몬스터정보`와 `/속성표`를 함께 시험할 수 있다.

Lv.200 이후 권역 표시명은 카이로스·아셴바흐·벨카인·루나리스·카미하라·아스트라·에버프로스트·라그나벨과 승천 권역의 고유명 중심으로 정리했다. Lv.500 이후 반복 미궁의 `상환/하환/합류핵/은닉고` 같은 압축 명칭도 `나선 윗길/아랫길/합류점/비밀 창고`처럼 위치를 바로 구분할 수 있는 표현으로 바꿨다. 장소·권역 내부 ID, 연결, 좌표, 방문 기록 key와 진행 조건은 유지하므로 기존 저장 데이터와 자동이동 경로는 호환된다.
...
`sendPlayerStats()`는 `Equipment.getDurabilityHudSnapshots()`의 무기·보호구 슬롯별 현재/최대 내구도를 500ms 변경 snapshot에 포함한다. 플레이어 상태 HUD 상세 설정은 `showArmorDurability`를 계정 설정과 이름 있는 서버 프리셋에 보존하며, 끄면 보호구 내구도 묶음만 숨기고 무기 내구도는 계속 표시한다. 화면 크기는 `displayPreferences.ts`가 단일 stable viewport로 갱신하며, 회전 중간 resize와 편집 중 키보드 높이를 구분해 가로↔세로 전환 후에도 CSS·HUD 좌표계를 일치시킨다. 저장된 px 좌표가 줄어든 화면을 벗어나면 영속 설정은 그대로 두고 렌더 좌표만 clamp한다. Location HUD의 행동 버튼은 편집기가 실제 focus된 동안 pointer focus 이동만 막아 모바일 화상 키보드를 유지하고, 뒤로가기로 닫힌 키보드를 다시 열기 위해 focus를 조작하지 않는다.
...
`sendLocationInfo()`는 현재 위치 좌표·위험도 라벨·PVP 허용 여부·인접 위치·동일 위치 플레이어·통합 `objects`의 생명력과 보호막, 현재 플레이어 기준 가능한 행동, NPC 표시 정보와 장소 기능 flag를 해당 사용자의 모든 소켓에 보낸다. 기능 flag는 실제 `location:fishing` 태그와 등록 상점 및 현재 플레이어의 카르마 접근 조건을 검사해 `낚시 가능`, `상점 이용 가능`으로 가공한다. 인접 위치는 `Location.getAvailableConnections(player)` 결과라 hidden은 제외되고 visible/locked 상태가 포함된다. NPC는 `Location.getNpcs()`와 `QuestBook.getNpcMarker()`의 가공된 이름·설명·퀘스트 표식만 전송한다. `Home.tsx`가 이를 `HudContext`에 저장하고 PlayerStatus/TargetStatus/Party/Location/Minimap HUD가 소비한다. TargetStatus HUD는 대상이 있을 때만 나타나는 독립 HUD이며 설정에서 표시·위치·크기를 조절하고 편집 모드에서는 미리보기를 표시한다. Location HUD는 기능 flag를 아이콘 칩으로 표시하고 서버가 보낸 `attack | target | interact` 행동만 버튼으로 만들므로 공격 불가 상자·질문 장치에는 상호작용만 표시하며, NPC 행에는 `/대화 번호` 버튼을 표시한다. PVP 가능 지역의 플레이어 행에는 `/대상지정p #고유번호` 버튼을 더한다. Minimap HUD는 세부 옵션을 켠 경우 visible 인접 지역만 이동 목록에 표시하고 `/이동 ID`를 숨김 실행한다.
