...
- 다음 레벨 요구 경험치는 `round(level * 100 * (1 + 3 * min(49, level - 1) / 49))`다. Lv.1은 100, Lv.50은 20,000, Lv.200은 80,000이며 일반 동급 몬스터 기준 보상 `level * 20`은 Lv.1에서 20%, Lv.50 이후 5%다. 보스는 긴 전투 시간을 반영해 별도 배율 경험치를 가질 수 있고 대장장이 제련·단조도 현재 `maxExp` 비율로 경험치를 준다. 레벨업마다 모든 Stat +1, 가용 statPoint +3이며 현재 Lv.345 월드에서도 별도 레벨 상한 없이 성장한다.
...
- Lv.235부터 끝없는 관측소 너머 잿빛성흔 심연에 진입한다. 25개 장소는 망자 골짜기·공허어금니/백골바람 분기, 흑염 회랑 순환로, 잿왕성 내성의 병영/성벽 분기로 구성된다. 회색불길 길잡이 타렌의 재료 복구 의뢰를 마치면 세 단계 보스 연속 토벌 의뢰가 열린다. Lv.248 세 아귀 문지기, 치유·보호·제어 위협을 우선하는 Lv.260 흑염대장 모르칸, 도발에 거의 흔들리지 않는 Lv.275 재왕 벨카르가 단계 관문을 이룬다. 봉인 예배당의 질문을 풀면 hidden 유산고와 7~10시간 유물함이 열린다.
- Lv.275부터 재왕의 왕좌 너머 공허왕관 성채에 진입한다. 25개 장소는 하층 안뜰의 성벽/정원, 외성문 뒤 주조실/기록고, 최상층의 관측소/법전고로 갈라졌다 다시 합쳐진다. Lv.290 무관성주 테오른은 공허창과 성벽 파단을 순서대로 사용하고, Lv.310 공허섭정 라시엘은 치유·보호·제어 위협을 계산해 세 기술을 무작위로 사용한다. 왕관 기둥 셋이 하나라도 남으면 라시엘이 받는 피해가 60% 줄어들며, 빈 왕좌의 서약을 풀면 hidden 금고와 첨탑 우회로가 열린다.
- Lv.310부터 공허왕관의 빈 왕좌 아래 침수 승강정으로 내려가 월식해구에 진입한다. 24개 장소는 밝은 산호초/침몰 선단, 해구숲/검은 역조, 백야성소의 성가회랑/조류기관실로 갈라졌다 합쳐진다. Lv.325 월조 리바이어던은 월조 해일과 해압 분쇄를 순서대로 사용하고, Lv.345 백야대사제 세르미아는 치유·보호·제어 위협을 계산해 백야와 월식 기술을 무작위로 사용한다. 조류거울 셋이 하나라도 남으면 세르미아가 받는 피해가 65% 줄어들며, 월식 조류제단의 답을 풀면 hidden 침수보물고 우회로가 열린다.
...
현재 월드는 총 209개 장소이며 다음 성장 동선을 제공한다. 성장 관문 보스는 Lv.15부터 최대 30레벨 간격으로 등장하고, 같은 레벨 일반 몬스터보다 최소 5배 경험치와 훨씬 높은 생명력을 가진다. 모든 보스가 미니게임을 강제하지 않으며 일부는 복수 SkillData 무작위 패턴·상태이상·주변 오브젝트 기믹으로 차별화한다.
...
| 잿빛성흔 심연·잿왕성 | Lv.238~275 분기형 심연 왕국 | 망자 골짜기·흑염 회랑·잿왕성 분기, Lv.248 세 아귀 문지기, Lv.260 흑염대장, 봉인 질문문과 숨은 유산고, Lv.275 재왕 |
| 공허왕관 성채 | Lv.275~310 다층 왕성 | 성벽/정원과 주조실/기록고 분기, Lv.290 무관성주, 빈 왕좌 서약과 숨은 금고, 기둥 셋을 먼저 파괴하는 Lv.310 공허섭정 |
| 월식해구·백야성소 | Lv.310~345 침수 성역 | 산호초/침몰선과 해구숲/역조 분기, Lv.325 월조 리바이어던, 조류제단과 숨은 보물고, 거울 셋을 먼저 파괴하는 Lv.345 백야대사제 |

고블린 정의와 배치는 제거됐다. 지역 관문은 `level_10/20/28/36/45/50/70/90/120/150/180/200/235/275/310` condition으로 잠기며, 관리자가 높은 지역으로 순간이동한 경우 되돌아올 수 있도록 역방향 연결은 열어 둔다. 일반·응축·수정·밤쇠·무광은·침은 광맥은 `tool:mining` 주무기만 공격할 수 있다. 몬스터 정의에는 불·물·얼음·자연·독·전기·돌·어둠·빛·언데드·신성·벌레·금속·땅 속성이 모두 실제 배치되어 `/몬스터정보`와 `/속성표`를 함께 시험할 수 있다.
