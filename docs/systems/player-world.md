...
- `/위치`는 정보 공개 모드를 따르며 기본적으로 본인에게만 보이고 공개 모드에서만 현재 채널에 공유된다. Monster와 Resource를 경계 없이 `[ 오브젝트 ]` 번호 목록으로 표시한다. 사망·파괴된 오브젝트는 체력 progress 대신 붉은 `(사망)`·`(파괴됨)` 상태를 표시한다. 등록된 상호작용 handler가 있는 살아 있는 오브젝트에는 `/상호작용 번호` 버튼을 붙이며, 나머지는 같은 명령을 실행해도 불가 알림을 보낸다.
