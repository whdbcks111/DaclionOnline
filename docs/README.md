...
| 버전별 패치노트 데이터·명령·화면 | [systems/patch-notes.md](systems/patch-notes.md) |
...
| 몬스터·보스·광물·탐험·요리 전문 도감 | [systems/codex.md](systems/codex.md) |
...
| 시스템 우편·아이템 첨부·원자 수령 | [systems/mailbox.md](systems/mailbox.md) |
...
| 반복 사냥 감지·사람 확인 | [systems/anti-automation.md](systems/anti-automation.md) |
...
6. 사용자에게 보이는 기능·콘텐츠·밸런스·오류 수정은 `shared/patchNotes.ts`의 현재 작업 묶음 버전에도 플레이어 관점으로 기록한다. 먼저 `.runtime/server-boot.json`을 읽고 `appliedPatchVersion`이 소스 최신 버전과 같으면 이미 적용된 버전이므로 파일의 `nextPatchVersion`으로 시작한다. 소스 최신 버전이 적용 버전보다 높고 직전 작업과 약 1시간 이내라면 아직 재부팅 전인 같은 버전에 합친다. 런타임 파일이 없거나 유효하지 않으면 재부팅 여부를 추측하지 않는다.
