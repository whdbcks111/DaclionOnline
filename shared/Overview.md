...
- `patchNotes.ts`: SemVer 버전별 사용자 공개 변경 기록, 배포일, 콘텐츠·시스템·밸런스·편의성·오류 수정 클래스형 분류와 버전 역순 불변 snapshot 조회 API. `v` 접두사를 정규화하며 서버 명령과 클라이언트 패치노트 화면이 같은 데이터를 사용한다.
