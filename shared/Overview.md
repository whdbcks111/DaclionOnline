...
- `types.ts`: 태그, `monster | resource` 통합 오브젝트·NPC ID·지도 아이콘·대표색이 포함된 LocationData, 플레이어별 `WorldMapData`와 `worldMap` ChatNode, 플레이어 레벨·상태효과·표시 가능한 스킬 쿨다운과 파티원 HP/MP HUD DTO, 정보 공개 모드·메시지·채널·온라인 mention completion Socket.io 양방향 이벤트 map의 단일 기준. `AdjacentLocationData`는 플레이어 기준 `visible | locked` 상태와 공개 잠금 사유를 선택적으로 포함한다.
