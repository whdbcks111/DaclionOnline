...
- `patchNotes.ts`: 작업 묶음별 SemVer 사용자 공개 변경 기록, 배포일, `[+] 기능/콘텐츠`, `[/] 수정`, `[-] 삭제` 클래스형 분류와 버전 역순 불변 snapshot 조회 API. 튜토리얼 완성까지는 `0.x.x` 베타로 구분하고, `v` 접두사를 정규화하며 서버 명령과 클라이언트 화면이 같은 데이터를 사용한다. 현재 v1.0.27은 전투 제어·재생·방어·몬스터 생명력·전투 기술 0.9초 연계 간격, 낚시 통신 오탐, 방어구 내구도·적대 구역 부활 대기, 보스 일일 의뢰·고급 대장장이 제작·속성 처치 통계·고레벨 희귀 광물 확률 조정을 안내한다.
