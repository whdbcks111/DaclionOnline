...
- `adaptiveMusic.ts`: 13개 `MusicScale`, 탐험·전투·보스 `MusicCombatState`, 서로 다른 음정·쉼 골격으로 재구성한 현재 월드 대표색 35개의 명시적 `LocationMusicTheme` 악보와 locationId seed 기반 scale-safe 4마디 A/독립 B 응답구 편곡, 7개 탐험 rhythm schedule, 8개 Omni timbre/envelope, 탐험 음역·밝기 mix, 음량·전환 정책과 기기 storage API. `composeLocationScore()`는 원 조성·4-step 구조음·화음 순서를 보존하고 lead/counter `67~96`, pad `60~84` 범위에서 전조·반복 시작점을 정규화해도 623개 장소의 pitch/rest/duration/onset 골격이 겹치지 않는 불변 MIDI schedule을 만들며 알 수 없는 색을 루미나르로 안전하게 폴백한다.
- `patchNotes.ts`: 작업 묶음별 SemVer 사용자 공개 변경 기록, 배포일, `[+] 기능/콘텐츠`, `[/] 수정`, `[-] 삭제` 클래스형 분류와 버전 역순 불변 snapshot 조회 API. 튜토리얼 완성까지는 `0.x.x` 베타로 구분하고, `v` 접두사를 정규화하며 서버 명령과 클라이언트 화면이 같은 데이터를 사용한다. 현재 v1.0.31은 관리자 전체 우편과 지역별 탐험 음악의 실제 선율·박자·음색 구분 강화를 안내한다.
