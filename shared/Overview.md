...
- `types.ts`: 태그, `monster | resource` 통합 오브젝트·NPC ID·지도 아이콘이 포함된 LocationData, 플레이어별 `WorldMapData`와 `worldMap` ChatNode, 플레이어 레벨·상태효과·표시 가능한 스킬 쿨다운과 파티원 HP/MP HUD DTO, 정보 공개 모드·메시지·채널 Socket.io 양방향 이벤트 map의 단일 기준.
