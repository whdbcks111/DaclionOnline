...
- `adaptiveMusic.ts`: 13개 `MusicScale`, 실제 `4/4·3/4` `MusicMeter`, 탐험·전투·보스 `MusicCombatState`, 현재 월드 대표색 35개의 고정 2마디 `motifA·responseA·motifB·cadence` 악구와 `A8–A′8–B8–A″8` 32마디 형식, 한 마디 단위 화성·chord-linked bass, 7개 rhythm, 8개 Omni timbre/envelope, 탐험 음역·밝기 mix, 음량·전환 정책과 기기 storage API. `composeLocationScore()`는 lead/counter `67~96`, pad `60~84`, PPQ 192 정수 tick schedule을 만들고 같은 권역의 훅은 보존한 채 locationId seed로 B구간과 voicing·BPM·타악 편곡만 구분하며 알 수 없는 색을 루미나르로 안전하게 폴백한다.
- `patchNotes.ts`: 작업 묶음별 SemVer 사용자 공개 변경 기록, 배포일, `[+] 기능/콘텐츠`, `[/] 수정`, `[-] 삭제` 클래스형 분류와 버전 역순 불변 snapshot 조회 API. 튜토리얼 완성까지는 `0.x.x` 베타로 구분하고, `v` 접두사를 정규화하며 서버 명령과 클라이언트 화면이 같은 데이터를 사용한다. 현재 v1.0.32는 지역 음악의 고정 악구·32마디 형식·실제 3/4·tick 동기화와 화성 개선을 안내한다.
