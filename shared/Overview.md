...
`PlayerStatsData.target`은 nullable 현재 대상의 HP/MP·보호막·상태이상과 감각 단계별 몬스터 분석 DTO를 공유한다. `tags.ts`의 `material:mana_crystal`은 원광·정제 소재·단조 결과가 같은 마력 재료 계보를 유지하게 한다.
