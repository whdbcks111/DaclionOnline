...
14. 사용자에게 보이는 기능·콘텐츠·밸런스·오류 수정은 같은 변경에서 `shared/patchNotes.ts`도 갱신한다. 수정 전에 `.runtime/server-boot.json`을 읽는다. 파일의 `appliedPatchVersion`이 소스의 최신 패치 버전과 같으면 그 버전은 이미 서버 재부팅으로 적용된 것이므로 파일의 `nextPatchVersion`으로 새 버전을 시작한다. 소스 최신 버전이 `appliedPatchVersion`보다 높으면 아직 재부팅 전 작업 묶음이므로 직전 작업 종료와 약 1시간 이내에는 같은 버전에 합친다. 파일이 없거나 유효하지 않으면 서버 재부팅 여부를 추측하지 말고 사용자에게 확인한다. 내부 구현명이 아닌 플레이어 관점의 `[+] 추가된 기능`, `[+] 추가된 콘텐츠`, `[/] 수정된 기능`, `[-] 삭제된 기능·콘텐츠` 형식으로 작성한다.
